...
<link rel="stylesheet" type="text/css" href="//ajax.googleapis.com/ajax/libs/jqueryui/1.11.3/themes/ui-lightness/jquery-ui.css" />
...
?>
				<li><i class="fa fa-sign-out"></i><a href="<?php echo site_url('login/logout?url=' . $this->uri->uri_string()); ?>" title="로그아웃">로그아웃</a></li>
				<li><i class="fa fa-user"></i><a href="<?php echo site_url('mypage'); ?>" title="마이페이지">마이페이지</a></li>
<?php } else { ?>
				<li><i class="fa fa-sign-in"></i><a href="<?php echo site_url('login?url=' . $this->uri->uri_string()); ?>" title="로그인">로그인</a></li>
...
<!-- 여기부터 레이아웃 하단입니다 -->
	...
	<footer>
		<div class="container">
			...
			<div  class="copyright">
				<?php if ($this->cbconfig->item('company_address')) { ?><span><?php echo  $this->cbconfig->item('company_address'); ?>
...
?>
			<div class="see_mobile"><a href="<?php echo current_url(); ?>?device=desktop" class="btn btn-primary btn-xs">PC 버전으로 보기</a></div>
<?php
} else {
	if ($this->cbconfig->get_device_type() == 'mobile') {
?>
			<div class="see_mobile"><a href="<?php echo current_url(); ?>?device=mobile" class="btn btn-primary btn-lg" style="width:100%;font-size:5em;">모바일 버전으로 보기</a></div>
<?php
	} else {
?>
			<div class="see_mobile"><a href="<?php echo current_url(); ?>?device=mobile" class="btn btn-primary btn-xs">모바일 버전으로 보기</a></div>
<?php
	}
}
?>
    ...
    <div id="side_menu">
        <div class="side_wr add_side_wr">
			...
			<?php if ($this->member->is_member()) { ?>
							<span><a href="<?php echo site_url('login/logout?url=' . $this->uri->uri_string()); ?>" class="btn btn-primary btn-xs" title="로그아웃"> <i class="fa fa-sign-out"></i> 로그아웃 </a></span>

							<span><a href="<?php echo site_url('mypage'); ?>" class="btn btn-primary btn-xs" title="마이페이지"> <i class="fa fa-user"></i> 마이페이지 </a></span>
			<?php } else { ?>
							<span><a href="<?php echo site_url('login?url=' . $this->uri->uri_string()); ?>" class="btn btn-primary btn-xs" title="로그인"> <i class="fa fa-sign-in"></i> 로그인 </a></span>
...
<script src="<?php echo base_url('assets/js/mobile.sidemenu.js'); ?>"></script>
<?php } ?>
