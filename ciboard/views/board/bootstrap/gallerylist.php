...
function ca_select($p='', $category='', $category_id='')
{
	...
	if ($p && is_array($p)) {
		foreach ($p as $result) {
			...
			if (element('bca_key', $result) == $category_id) $return .= 'selected="selected"';
...
<?php if (element('use_category' , element('board', element('list',$view))) && element('cat_display_style' , element('board', element('list',$view))) == 'tab') { 
$category = element('category', element('board', element('list',$view)));
?>
...
?>
 ...
 <?php if (element('is_admin', $view)) { ?><div><label for="all_boardlist_check"><input id="all_boardlist_check" onclick="if (this.checked) all_boardlist_checked(true); else all_boardlist_checked(false);" type="checkbox"> 전체선택</label></div><?php } ?>
						...
						<tr>
							...
							<td>
								...
								<a href="<?php echo element('post_url', $result); ?>" style="
									<?php
										if (element('post_id', element('post', $view)) == element('post_id', $result)) { echo 'font-weight:bold;'; }
									?>
