...
<?php echo show_alert_message(element('message', $view), '<div class="alert alert-auto-close alert-dismissible alert-info"><button type="button" class="close alertclose" >&times;</button>', '</div>'); ?>
<?php
...
function ca_select($p='', $category='', $post_category='')
{
	...
	if ($p && is_array($p)) {
		foreach ($p as $result) {
			...
			if (element('bca_key', $result) == $post_category) $return .= 'selected="selected"';
...
<?php if (element('use_dhtml', element('board', $view)) == '' AND (element('post_min_length', element('board', $view)) OR element('post_max_length', element('board', $view)))) { ?>
				<div class="well well-sm" style="margin:15px 0;">
					...
					<?php }
						if (element('post_max_length', element('board', $view))) { ?>
...
<?php
	if ( is_numeric(element('link_count', element('board', $view))) && element('link_count', element('board', $view)) > 0) {
