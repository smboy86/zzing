...
class _Common {
	...
	function init()
	{
		...
		if ($CI->uri->segment(1) == 'install') return;

		if ($CI->config->item('use_lock_ip') && $CI->cbconfig->item('site_ip_whitelist')) {
		...
		if ($CI->config->item('use_lock_ip') && $CI->cbconfig->item('site_ip_blacklist')) {
		...
		if ( ! $device_view_type) $device_view_type = (get_cookie('device_view_type') == 'desktop' OR get_cookie('device_view_type') == 'mobile') ? get_cookie('device_view_type') : '';
		if ( ! $device_view_type) $device_view_type = $detect->isMobile() ? 'mobile' : 'desktop';
		...
		if (get_cookie('autologin') && $CI->session->userdata('mem_id') == '') {
			...
			$autodata = $CI->Autologin_model->get_one('', '', $where = array('aul_key' => get_cookie('autologin')));
			if ( ! element('mem_id', $autodata)) {
				delete_cookie('autologin');
			} else if ( ! element('aul_datetime', $autodata) OR ( strtotime(element('aul_datetime', $autodata)) < ctimestamp() * 86400 * 30)) {
				$CI->Autologin_model->delete(element('aul_id', $autodata));
				delete_cookie('autologin');
			} else if ($CI->input->ip_address() != element('aul_ip', $autodata)) {
				$CI->Autologin_model->delete(element('aul_id', $autodata));
				delete_cookie('autologin');
			} else {
				...
				if ( ! element('mem_id', $tmpmember)) {
					$CI->Autologin_model->delete(element('aul_id', $autodata));
					delete_cookie('autologin');
				} else if (element('mem_denied', $tmpmember)) {
					$CI->Autologin_model->delete(element('aul_id', $autodata));
					delete_cookie('autologin');
				} else if (element('mem_is_admin', $tmpmember)) {
					$CI->Autologin_model->delete(element('aul_id', $autodata));
					delete_cookie('autologin');
				} else {
			...
			if ($CI->member->item('mem_denied')) {
				...
				if (substr($CI->member->item('mem_lastlogin_datetime'), 0, 10) != cdate('Y-m-d')) {
					if ($CI->cbconfig->item('point_login')) {
						...
						$point = $CI->point->insert_point($mem_id, $CI->cbconfig->item('point_login'), cdate('Y-m-d') . ' 첫로그인', 'login' , $mem_id, cdate('Y-m-d') . ' 로그인');
					}
					$updatedata = array(
		...
		if ($CI->config->item('use_lock_ip') && $CI->uri->segment(1) == config_item('uri_segment_admin') && $CI->cbconfig->item('admin_ip_whitelist')) {
