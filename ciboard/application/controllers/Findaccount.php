...
class Findaccount extends CB_Controller
{
	...
	*  모델을 로딩합니다
	*/
	protected $models = array('Member_login_log', 'Member_auth_email');
	...
	public function index()
	{
		...
		*/
		...
		if ($this->input->post('findtype') == 'findidpw') {
			$config[] = array('field'=>'idpw_email', 'label'=>'이메일', 'rules'=>'trim|required|valid_email|callback__existemail');
		} else if ($this->input->post('findtype') == 'verifyemail') {
			$config[] = array('field'=>'verify_email', 'label'=>'이메일', 'rules'=>'trim|required|valid_email|callback__verifyemail');
		} else if ($this->input->post('findtype') == 'changeemail') {
			$config[] = array('field'=>'change_userid', 'label'=>'아이디', 'rules'=>'trim|required|alphanumunder|min_length[3]|max_length[20]');
			$config[] = array('field'=>'change_password', 'label'=>'패스워드', 'rules'=>'trim|required|callback__check_id_pw[' . $this->input->post('change_userid') . ']');
			$config[] = array('field'=>'change_email', 'label'=>'이메일', 'rules'=>'trim|required|valid_email|callback__change_email');
		}
		...
		*/
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->input->post('findtype') == 'findidpw') {
				...
				$vericode = array('$', '/' , '.');
				$verificationcode = str_replace($vericode, '', password_hash($mem_id . '-' . $this->input->post('idpw_email') . '-' . random_string('alnum', 10) , PASSWORD_BCRYPT));
				...
				$title = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_findaccount_user_title'));
				$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_findaccount_user_content'));
				...
				$vericode = array('$', '/' , '.');
				$verificationcode = str_replace($vericode, '', password_hash($mem_id . '-' . $this->input->post('verify_email') . '-' . random_string('alnum', 10) , PASSWORD_BCRYPT));
				...
				$title = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_resendverify_user_title'));
				$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_resendverify_user_content'));
				...
				$vericode = array('$', '/' , '.');
				$verificationcode = str_replace($vericode, '', password_hash($mem_id . '-' . $this->input->post('change_email') . '-' . random_string('alnum', 10) , PASSWORD_BCRYPT));
	...
	*/
	public function _existemail($str)
	{
		...
		$userinfo = $this->Member_model->get_by_email($str, 'mem_id, mem_email, mem_denied, mem_email_cert');
		if ( ! element('mem_id', $userinfo)) {
			$this->form_validation->set_message('_existemail', '존재하지 않는 이메일주소입니다');
			return FALSE;
		}
		if (element('mem_denied', $userinfo) == '1') {
			$this->form_validation->set_message('_existemail', '회원님의 아이디는 탈퇴하신 아이디입니다');
			return FALSE;
		} else if (element('mem_denied', $userinfo) == '2') {
			$this->form_validation->set_message('_existemail', '회원님의 아이디는 관리자에 의해 차단되었습니다');
			return FALSE;
		} else if ($this->cbconfig->item('use_register_email_auth') && ! element('mem_email_cert', $userinfo)) {
			$this->form_validation->set_message('_existemail', '회원님은 아직 이메일 인증을 받지 않으셨습니다<br> 아래 인증메일 재발송 란에서 이메일을 받아 인증해주시기 바랍니다');
			return FALSE;
		}
		return TRUE;
	...
	*/
	public function _verifyemail($str)
	{
		...
		$userinfo = $this->Member_model->get_by_email($str, 'mem_id, mem_email, mem_denied');
		if ( ! element('mem_id', $userinfo)) {
			$this->form_validation->set_message('_verifyemail', '존재하지 않는 이메일주소입니다');
			return FALSE;
		}
		if (element('mem_denied', $userinfo) == '1') {
			$this->form_validation->set_message('_verifyemail', '회원님의 아이디는 탈퇴하신 아이디입니다');
			return FALSE;
		} else if (element('mem_denied', $userinfo) == '2') {
			$this->form_validation->set_message('_verifyemail', '회원님의 아이디는 관리자에 의해 차단되었습니다');
			return FALSE;
		}
		return TRUE;
	...
	*/
	public function _change_email($str)
	{
		$userinfo = $this->Member_model->get_by_email($str, 'mem_id, mem_email, mem_denied');
		if (element('mem_id', $userinfo)) {
			$this->form_validation->set_message('_change_email', '이 이메일은 이미 다른 회원에 의해 사용되어지고 있는 이메일입니다');
			return FALSE;
		}
	}
	...
	*/
	public function _check_id_pw($password, $userid)
	{
		...
		$max_login_try_count = $this->cbconfig->item('max_login_try_count');
		$max_login_try_limit_second = $this->cbconfig->item('max_login_try_limit_second');
		...
		if ($max_login_try_count && $max_login_try_limit_second) {
			...
			$logindata = $this->Member_login_log_model->get('', 'mll_id, mll_success, mem_id, mll_ip, mll_datetime', $where, '', '', 'mll_id', 'DESC');
			...
			if ($logindata && is_array($logindata)) {
				foreach ($logindata as $key => $val) {
					...
					if (element('mll_success', $val) == 1) break;
				}
			}
		...
		$userinfo = $this->Member_model->get_by_userid($userid, 'mem_id, mem_password, mem_denied');
		...
		if ( ! element('mem_id', $userinfo) OR ! element('mem_password', $userinfo)) {
			...
			$this->_update_login_log(0, 0, '회원아이디가 존재하지 않습니다');
			return FALSE;
		} else if ( ! password_verify($password, element('mem_password', $userinfo))) {
			$this->form_validation->set_message('_check_id_pw', '회원 아이디와 패스워드가 서로 맞지 않습니다' . $loginfailmessage);
			$this->_update_login_log(element('mem_id', $userinfo), 0, '패스워드가 올바르지 않습니다');
			return FALSE;
		} else if (element('mem_denied', $userinfo)) {
			if (element('mem_denied', $userinfo) == '1') {
				$this->form_validation->set_message('_check_id_pw', '회원님의 아이디는 탈퇴하신 아이디입니다');
				$this->_update_login_log(element('mem_id', $userinfo), 0, '탈퇴한 회원아이디입니다');
				return FALSE;
			} else if (element('mem_denied', $userinfo) == '2') {
				$this->form_validation->set_message('_check_id_pw', '회원님의 아이디는 관리자에 의해 차단되었습니다');
				$this->_update_login_log(element('mem_id', $userinfo), 0, '차단된 회원아이디입니다');
				return FALSE;
			}
		}
	...
	* 로그인 기록을 남깁니다
	*/
	public function _update_login_log($mem_id=0 , $success=0, $reason='')
	{
		$loginlog = array(
			'mll_success' => $success,
			'mem_id' => $mem_id	,
			...
			'mll_referer' => $this->input->get_post('url'),
		);
