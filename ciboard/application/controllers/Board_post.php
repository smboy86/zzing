...
class Board_post extends CB_Controller
{
    ...
    public function post($post_id = 0, $print = false)
    {
        ...
        if ($mem_id > 0  && $mem_id !== (int) element('mem_id', $post)
        ...
        if (element('use_personal', $board) && $is_admin === false
            && $mem_id !== (int) element('mem_id', $post)) {
        ...
        if (element('post_del', $post)) {
            ...
            if ($is_blind === false OR $is_admin !== false
                OR (element('mem_id', $post) && (int) element('mem_id', $post) === $mem_id)) {
        ...
        $can_modify = ($is_admin !== false OR ($mem_id > 0 && $mem_id === (int) element('mem_id', $post))
            OR ! element('mem_id', $post)) ? true : false;
        $can_delete = ($is_admin !== false OR ($mem_id > 0 && $mem_id === (int) element('mem_id', $post))
            OR ! element('mem_id', $post)) ? true : false;
