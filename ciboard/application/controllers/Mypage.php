...
class Mypage extends CB_Controller
{
	...
	*/
	public function post()
	{
		...
		*/
		$where = array('post.mem_id' => $this->member->item('mem_id'), 'post_del' => 0);
	...
	*/
	public function comment()
	{
		...
		* 게시판 목록에 필요한 정보를 가져옵니다.
		*/
		$where = array('comment.mem_id' => $this->member->item('mem_id'));
		$result = $this->Comment_model->get_comment_list($per_page, $offset, $where , '' , $findex, $forder);
		$list_num = $result['total_rows'] - ($page - 1) * $per_page;
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
	...
	*/
	public function point()
	{
		...
		* 게시판 목록에 필요한 정보를 가져옵니다.
		*/
		$where = array('point.mem_id' => $this->member->item('mem_id'));
		$result = $this->Point_model->get_list($per_page, $offset, $where , '' , $findex, $forder);
		$list_num = $result['total_rows'] - ($page - 1) * $per_page;
		$result['plus'] = 0;
		$result['minus'] = 0;
		if (element('list', $result)) {
	...
	*/
	public function followinglist()
	{
		...
		* 게시판 목록에 필요한 정보를 가져옵니다.
		*/
		$where = array('follow.mem_id' => $this->member->item('mem_id'));
		$result = $this->Follow_model->get_following_list($per_page, $offset, $where , '' , $findex, $forder);
		$list_num = $result['total_rows'] - ($page - 1) * $per_page;
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
		...
		$view['view']['followed_total_rows'] = $this->Follow_model->count_by($where = array('target_mem_id' => $this->member->item('mem_id')));
	...
	*/
	public function followedlist()
	{
		...
		* 게시판 목록에 필요한 정보를 가져옵니다.
		*/
		$where = array('follow.target_mem_id' => $this->member->item('mem_id'));
		...
		$view['view']['following_total_rows'] = $this->Follow_model->count_by($where = array('mem_id' => $this->member->item('mem_id')));
	...
	*/
	public function like_post()
	{
		...
		* 게시판 목록에 필요한 정보를 가져옵니다.
		*/
		$where = array('like.mem_id' => $this->member->item('mem_id'), 'lik_type' => '1', 'target_type' => '1', 'post.post_del' =>0);
		$result = $this->Like_model->get_post_like_list($per_page, $offset, $where , '' , $findex, $forder);
		$list_num = $result['total_rows'] - ($page - 1) * $per_page;
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				...
				$images = '';
				if (element('post_image', $val)) {
					$images = $this->Post_file_model->get_one('' , '', array('post_id' => element('post_id', $val), 'pfi_is_image' => 1), '' , '', 'pfi_id', 'ASC');
				}
	...
	*/
	public function like_comment()
	{
		...
		* 게시판 목록에 필요한 정보를 가져옵니다.
		*/
		$where = array('like.mem_id' => $this->member->item('mem_id'), 'lik_type' => '1', 'target_type' => '2');
		$result = $this->Like_model->get_comment_like_list($per_page, $offset, $where , '' , $findex, $forder);
		$list_num = $result['total_rows'] - ($page - 1) * $per_page;
	...
	*/
	public function scrap()
	{
		...
		*/
		$config = array(
			array('field'=>'scr_id', 'label'=>'SCRAP ID', 'rules'=>'trim|required|numeric'),
			array('field'=>'scr_title', 'label'=>'제목', 'rules'=>'trim'),
		);
		...
		*/
		...
		if ($this->form_validation->run() == FALSE) {
			...
			*/
			...
			$updatedata = array(
				'scr_title' => $this->input->post('scr_title'),
			);
		...
		* 게시판 목록에 필요한 정보를 가져옵니다.
		*/
		$where = array('scrap.mem_id' => $this->member->item('mem_id'), 'post.post_del' => 0);
	...
	*  마이페이지>스크랩삭제 입니다
	*/
	public function scrap_delete($scr_id='')
	{
		...
		if ( ! is_numeric($scr_id) OR $scr_id < 1) {
			show_404();
		}
		...
		if (element('mem_id', $scrap) != $this->member->item('mem_id')) {
			show_404();
		}
	...
	*/
	public function loginlog()
	{
		...
		* 게시판 목록에 필요한 정보를 가져옵니다.
		*/
		$where = array('mem_id' => $this->member->item('mem_id'));
