...
class Login extends CB_Controller
{
	...
	*  모델을 로딩합니다
	*/
	protected $models = array('Member_login_log', 'Member_meta', 'Autologin');
	...
	public function index()
	{
		...
		*/
		if ($use_login_account== 'both') {
			$config[] = array('field'=>'mem_userid', 'label'=>'아이디 또는 이메일', 'rules'=>'trim|required');
			$view['view']['userid_label_text'] = '아이디 또는 이메일';
		} else if ($use_login_account== 'email') {
			$config[] = array('field'=>'mem_userid', 'label'=>'이메일', 'rules'=>'trim|required|valid_email');
			$view['view']['userid_label_text'] = '이메일';
		} else {
			$config[] = array('field'=>'mem_userid', 'label'=>'아이디', 'rules'=>'trim|required|alphanumunder|min_length[3]|max_length[20]');
			$view['view']['userid_label_text'] = '아이디';
		}
		$config[] = array('field'=>'mem_password', 'label'=>'패스워드', 'rules'=>'trim|required|min_length[4]|callback__check_id_pw[' . $this->input->post('mem_userid') . ']');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->input->post('returnurl')) {
				if (validation_errors('<div class="alert alert-warning" role="alert">', '</div>')) {
					$this->session->set_flashdata('loginvalidationmessage', validation_errors('<div class="alert alert-warning" role="alert">', '</div>'));
				}
				$this->session->set_flashdata('loginuserid', $this->input->post('mem_userid'));
			...
			*/
			...
			$this->_update_login_log(element('mem_id', $userinfo), 1, '로그인 성공');
			$this->session->set_userdata('mem_id' , element('mem_id', $userinfo));

			if ($this->input->post('autologin')) {
				$vericode = array('$', '/' , '.');
				$hash = str_replace($vericode, '', password_hash(random_string('alnum', 10) . element('mem_id', $userinfo) . ctimestamp() . element('mem_userid', $userinfo) , PASSWORD_BCRYPT));
			...
			if ($change_password_date) {
				...
				if ( ctimestamp() - strtotime($meta_change_pw_datetime) > $change_password_date * 86400) {
					...
					$this->session->set_flashdata('message', html_escape($site_title) . ' 은(는) 회원님의 비밀번호를 주기적으로 변경하도록 권장합니다.<br /> 오래된 비밀번호를 사용중인 회원님께서는 안전한 서비스 이용을 위해 비밀번호 변경을 권장합니다');
			...
			if ( ! $url_after_login) {
				$url_after_login = $this->input->get_post('url') ? $this->input->get_post('url') : site_url();
			}
	...
	*/
	public function _check_id_pw($password, $userid)
	{
		...
		if ($max_login_try_count && $max_login_try_limit_second) {
			...
			if ($logindata && is_array($logindata)) {
				foreach ($logindata as $key => $val) {
					if ($val['mll_success'] == 0) {
						$loginfailnum++;
					}
					if ($val['mll_success'] == 1) break;
				}
			}
			if ($loginfailnum > 0 && $loginfailnum % $max_login_try_count == 0) {
				...
				if ($next_login > 0) {
					$this->form_validation->set_message('_check_id_pw', '회원님은 패스워드를 연속으로 ' . $loginfailnum . '회 잘못 입력하셨기 때문에 ' . $next_login . '초 후에 다시 로그인 시도가 가능합니다');
					return FALSE;
				}
			}
		...
		if ($use_login_account== 'both') {
			$userinfo = $this->Member_model->get_by_both($userid, 'mem_id, mem_password, mem_denied, mem_email_cert, mem_is_admin');
		} else if ($use_login_account== 'email') {
			$userinfo = $this->Member_model->get_by_email($userid, 'mem_id, mem_password, mem_denied, mem_email_cert, mem_is_admin');
		} else {
			$userinfo = $this->Member_model->get_by_userid($userid, 'mem_id, mem_password, mem_denied, mem_email_cert, mem_is_admin');
		}
		...
		if ( ! element('mem_id', $userinfo) OR ! element('mem_password', $userinfo)) {
			...
			$this->_update_login_log(0, 0, '회원 아이디가 존재하지 않습니다');
			return FALSE;
			...
			$this->form_validation->set_message('_check_id_pw', '회원님은 아직 이메일 인증을 받지 않으셨습니다');
			$this->_update_login_log(element('mem_id', $userinfo), 0, '이메일 인증을 받지 않은 회원아이디입니다');
			return FALSE;
		} else if (element('mem_is_admin', $userinfo) && $this->input->post('autologin')) {
	...
	*/
	public function logout()
	{
		...
		if ($this->member->is_member() == FALSE) redirect();

		$where = array(
			'mem_id' => 	$this->member->item('mem_id'),
		);
		$this->Autologin_model->delete('', $where);
