...
class Login extends CB_Controller
{
     ...
     * 모델을 로딩합니다
     */
    protected $models = array('Member_login_log', 'Member_meta');
    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() === false) {
            ...
            $this->member->update_login_log(element('mem_id', $userinfo), 1, '로그인 성공');
    ...
    public function _check_id_pw($password, $userid)
    {
        ...
        if ( ! element('mem_id', $userinfo) OR ! element('mem_password', $userinfo)) {
            ...
            $this->member->update_login_log(0, 0, '회원 아이디가 존재하지 않습니다');
            return false;
            ...
            $this->member->update_login_log(element('mem_id', $userinfo), 0, '이메일 인증을 받지 않은 회원아이디입니다');
            return false;
