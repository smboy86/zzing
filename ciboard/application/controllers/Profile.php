...
class Profile extends CB_Controller
{
	...
	*/
 	public function index($userid = '')
	{
		...
		$view['view']['member'] = $member = array_merge($view['view']['member'], $this->Member_meta_model->get_all_meta(element('mem_id', $member)));
		...
		$view['view']['member'] = $member = array_merge($view['view']['member'], $this->Member_extra_vars_model->get_all_meta(element('mem_id', $member)));
	...
	*/
	public function sideview($userid = '')
	{
		...
		$member = $this->Member_model->get_by_userid($userid, 'mem_id, mem_nickname, mem_homepage, mem_receive_email, mem_use_note, mem_open_profile, mem_denied  ');
		if ( ! element('mem_id', $member)) {
		...
		$followcount = $this->Follow_model->count_by(array('mem_id'=> $this->member->item('mem_id'), 'target_mem_id' => element('mem_id', $member)));
		$result = array();
		...
		} else if ($this->cbconfig->get_device_view_type() == 'mobile' && $this->cbconfig->item('use_mobile_sideview_email') == '') {
			$result['email']		= '10'; // 이메일 기능을 사용하지 않는 사이트
		} else if ( ! $this->member->item('mem_receive_email') && $this->member->is_admin() != 'super') {
			$result['email'] = '3'; // 내가 이메일수신 기능을 사용하지 않고 있음
		} else if ($member['mem_receive_email'] OR  $this->member->is_admin() == 'super') {
			$result['email'] = '1'; // 이메일수신 기능 사용
		} else {
			$result['email'] = '2'; // 상대방이 이메일수신 기능을 사용하지 않고 있음
		}

		if ( ! $this->member->item('mem_open_profile') && $this->member->is_admin() != 'super') {
			$result['profile'] = '3'; // 내가 프로필공개 기능을 사용하지 않고 있음
		} else if ($member['mem_open_profile'] OR  $this->member->is_admin() == 'super') {
	...
	*/
	public function add_follow($userid = '')
	{
		...
		$target = $this->Member_model->get_by_userid($userid, 'mem_id, mem_nickname, mem_homepage, mem_receive_email, mem_use_note, mem_open_profile, mem_denied  ');
		...
		if (element('mem_id', $target) == $this->member->item('mem_id')) {
			$result = array('error' => '자기자신을 친구로 등록할 수 없습니다');
			exit(json_encode($result));
		}
		$followcount = $this->Follow_model->count_by(array('mem_id'=> $this->member->item('mem_id'), 'target_mem_id' => element('mem_id', $target)));
		if ($followcount > 0) {
		...
		$target_count = $this->Follow_model->count_by(array('target_mem_id' => element('mem_id', $target)));
		$my_count = $this->Follow_model->count_by(array('mem_id' => $this->member->item('mem_id')));

		$this->Member_model->update(element('mem_id', $target), array('mem_followed' => $target_count));
		$this->Member_model->update($this->member->item('mem_id'), array('mem_following' => $my_count));
	...
	*/
	public function delete_follow($userid = '')
	{
		...
		$this->Follow_model->delete('', array(
			'mem_id' => 	$this->member->item('mem_id'),
			'target_mem_id' => element('mem_id', $target),
		));
