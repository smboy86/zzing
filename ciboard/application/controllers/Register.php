...
class Register extends CB_Controller
{
	...
	*  모델을 로딩합니다
	*/
	protected $models = array('Member_nickname', 'Member_meta', 'Member_auth_email', 'Member_register', 'Note', 'Member_extra_vars');
	...
	*/
	...
	public function index()
	{
		...
		if($this->member->is_member() && !($this->member->is_admin() && $this->uri->segment(1) == config_item('uri_segment_admin'))) {
			redirect();
		}
		...
		*/
		$config = array(
			array('field'=>'agree', 'label'=>'회원가입약관', 'rules'=>'trim|required'),
			array('field'=>'agree2', 'label'=>'개인정보취급방침', 'rules'=>'trim|required')
		);
	...
	*/
	public function form()
	{
		...
		if ($this->cbconfig->item('change_nickname_date')) {
			$nickname_description = '<br />닉네임을 입력하시면 앞으로 ' . $this->cbconfig->item('change_nickname_date') . '일 이내에는 변경할 수 없습니다';
		}

		$configbasic['mem_userid'] = array('field'=>'mem_userid', 'label'=>'아이디', 'rules'=>'trim|alphanumunder|min_length[3]|max_length[20]|is_unique[member.mem_userid]|callback__mem_userid_check', 'description' => '영문자, 숫자, _ 만 입력 가능. 최소 3자이상 입력하세요');
		...
		$configbasic['mem_password'] = array('field'=>'mem_password', 'label'=>'패스워드', 'rules'=>'trim|min_length[' . $password_length . ']|callback__mem_password_check', 'description' => $password_description);
		$configbasic['mem_password_re'] = array('field'=>'mem_password_re', 'label'=>'패스워드 확인', 'rules'=>'trim|required|min_length[' . $password_length . ']|matches[mem_password]');
		...
		$configbasic['mem_email'] = array('field'=>'mem_email', 'label'=>'이메일', 'rules'=>'trim|valid_email|max_length[50]|is_unique[member.mem_email]|callback__mem_email_check', 'description' => $email_description);
		...
		$configbasic['mem_recommend'] = array('field'=>'mem_recommend', 'label'=>'추천인아이디', 'rules'=>'trim|alphanumunder|min_length[3]|max_length[20]|callback__mem_recommend_check');

		if ( $this->member->is_admin() == FALSE && ! $this->session->userdata('registeragree')) {

			$this->session->set_flashdata('message', '회원가입약관동의와 개인정보취급방침동의후 회원가입이 가능합니다');
			redirect('register');
		}
		...
		*/
		if ($form_validation == FALSE OR $file_error != '' OR $file_error2 != '') {
				...
				foreach ($form as $key => $value) {
					...
					if (element('field_type', $value) == 'text' OR element('field_type', $value) == 'url' OR element('field_type', $value) == 'email' OR element('field_type', $value) == 'phone' OR element('field_type', $value) == 'date') {
						...
						$html_content[$k]['input'] .= '
						  ...
						  ';
					} else if (element('field_name', $value) == 'mem_password') {
			...
			*/
			...
			$mem_level = $this->cbconfig->item('register_level');
			$insertdata = array();
			$metadata = array();
			...
			if ( isset($form['mem_homepage']['use']) && $form['mem_homepage']['use']) {
				$insertdata['mem_homepage'] = $this->input->post('mem_homepage');
			}
			if ( isset($form['mem_phone']['use']) && $form['mem_phone']['use']) {
				$insertdata['mem_phone'] = $this->input->post('mem_phone');
			}
			if ( isset($form['mem_birthday']['use']) && $form['mem_birthday']['use']) {
				$insertdata['mem_birthday'] = $this->input->post('mem_birthday');
			}
			if ( isset($form['mem_sex']['use']) && $form['mem_birthday']['use']) {
				$insertdata['mem_sex'] = $this->input->post('mem_sex');
			}
			if ( isset($form['mem_address']['use']) && $form['mem_address']['use']) {
				$insertdata['mem_zipcode'] = $this->input->post('mem_zipcode');
				$insertdata['mem_address1'] = $this->input->post('mem_address1');
				$insertdata['mem_address2'] = $this->input->post('mem_address2');
				$insertdata['mem_address3'] = $this->input->post('mem_address3');
				$insertdata['mem_address4'] = $this->input->post('mem_address4');
			}
			$insertdata['mem_receive_email'] = $this->input->post('mem_receive_email');
			if ($this->cbconfig->item('use_note')) {
				$insertdata['mem_use_note'] = $this->input->post('mem_use_note');
				$metadata['meta_use_note_datetime'] = cdate('Y-m-d H:i:s');
			}
			$insertdata['mem_receive_sms'] = $this->input->post('mem_receive_sms');
			$insertdata['mem_open_profile'] = $this->input->post('mem_open_profile');
			...
			if ( isset($form['mem_profile_content']['use']) && $form['mem_profile_content']['use']) {
				$insertdata['mem_profile_content'] = $this->input->post('mem_profile_content');
			}
			...
			$point = $this->point->insert_point($mem_id, $this->cbconfig->item('point_register'), '회원가입을 축하합니다', 'member' , $mem_id, '회원가입');
			...
			$replaceconfig = array(
				...
				site_url(),
				html_escape($this->input->post('mem_userid')),
				html_escape($this->input->post('mem_nickname')),
				html_escape($this->input->post('mem_username')),
			...
			if ( ! $this->cbconfig->item('use_register_email_auth')) {
				if (($this->cbconfig->item('send_email_register_user') && $this->input->post('mem_receive_email')) OR $this->cbconfig->item('send_email_register_alluser')) {
					$title = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_register_user_title'));
					$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_register_user_content'));
				...
				$title = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_register_user_verifytitle'));
				$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_register_user_verifycontent'));
				...
				$this->session->set_flashdata('email_auth_message', $email_auth_message);
			...
			$superadminlist = '';
			if ($this->cbconfig->item('send_email_register_admin') OR $this->cbconfig->item('send_note_register_admin')) {
			...
			if ($this->cbconfig->item('send_email_register_admin') OR $this->cbconfig->item('send_note_register_admin')) {
				$adminlist = $this->Member_model->get('', 'mem_id, mem_email, mem_nickname, mem_phone', array('mem_is_admin' => 1, 'mem_denied' => 0));
			}
			if ($emailsendlistadmin) {
				$title = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_register_admin_title'));
				$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_register_admin_content'));
			...
			if ($notesendlistadmin) {
				$title = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_register_admin_title'));
				$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_register_admin_content'));
			...
			if ($notesendlistuser && element('mem_id', $notesendlistuser)) {
				$title = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_register_user_title'));
				$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_register_user_content'));
			...
			if ($this->input->post('mem_recommend')) {
				if ($this->cbconfig->item('point_recommended')) {
					// 추천인이 존재할 경우 추천된 사람
					$this->point->insert_point(element('mem_id', $recommended), $this->cbconfig->item('point_recommended'), $this->input->post('mem_nickname') . ' 님이 회원가입시 추천함', 'member_recommended' , $mem_id, '회원가입추천');
				}
				if ($this->cbconfig->item('point_recommender')) {
					// 추천인이 존재할 경우 가입자에게
					$this->point->insert_point($mem_id, $this->cbconfig->item('point_recommender'), '회원가입 추천인존재', 'member_recommender' , $mem_id, '회원가입추천인존재');
				}
			}

			$this->session->set_flashdata('nickname', $this->input->post('mem_nickname'));
	...
	*/
	...
	public function ajax_userid_check()
	{
		...
		if( ! $userid) {
			$result = array('result' => 'no', 'reason' => '아이디값이 넘어오지 않았습니다');
			exit(json_encode($result));
		}

		if( ! preg_match("/^([a-z0-9_])+$/i", $userid)) {
			$result = array('result' => 'no', 'reason' => '아이디는 숫자, 알파벳, _ 만 입력가능합니다');
			exit(json_encode($result));
		}
		...
		if( $count > 0) {
			$result = array('result' => 'no', 'reason' => '이미 사용중인 아이디입니다');
			exit(json_encode($result));
		}

		if($this->_mem_userid_check($userid) == FALSE) {
			$result = array('result' => 'no', 'reason' => $userid . '은(는)  예약어로 사용하실 수 없는 회원아이디입니다');
			exit(json_encode($result));
		}
		...
		$result = array('result' => 'available', 'reason' => '사용 가능한 아이디입니다');
		exit(json_encode($result));
	}
	...
	public function ajax_email_check()
	{
		...
		if( ! $email) {
			$result = array('result' => 'no', 'reason' => '이메일값이 넘어오지 않았습니다');
			exit(json_encode($result));
		}

		if($this->member->item('mem_email') && $this->member->item('mem_email') == $this->input->post('email')) {
			$result = array('result' => 'available', 'reason' => '사용 가능한 이메일입니다');
			exit(json_encode($result));
		}
		...
		$count = $this->Member_model->count_by($where);
		if( $count > 0) {
			$result = array('result' => 'no', 'reason' => '이미 사용중인 이메일입니다');
			exit(json_encode($result));
		}

		if($this->_mem_email_check($email) == FALSE) {
			$result = array('result' => 'no', 'reason' => $email . '은(는)  예약어로 사용하실 수 없는 이메일입니다');
			exit(json_encode($result));
		}
	...
	public function ajax_password_check()
	{
		...
		if( ! $password) {
			$result = array('result' => 'no', 'reason' => '패스워드값이 넘어오지 않았습니다');
			exit(json_encode($result));
		}
		
		if($this->_mem_password_check($password) == FALSE) {
			$result = array('result' => 'no', 'reason' => '패스워드를 다시 입력해주세요');
			exit(json_encode($result));
		}

		$result = array('result' => 'available', 'reason' => '사용 가능한 패스워드입니다');
		exit(json_encode($result));
	}
	...
	public function ajax_nickname_check()
	{
		...
		if( ! $nickname) {
			$result = array('result' => 'no', 'reason' => '닉네임값이 넘어오지 않았습니다');
			exit(json_encode($result));
		}

		if($this->member->item('mem_nickname') && $this->member->item('mem_nickname') == $this->input->post('nickname')) {
			$result = array('result' => 'available', 'reason' => '사용 가능한 닉네임입니다');
			exit(json_encode($result));
		}
		...
		$count = $this->Member_model->count_by($where);
		if( $count > 0) {
			$result = array('result' => 'no', 'reason' => '이미 사용중인 닉네임입니다');
			exit(json_encode($result));
		}
	...
	*/
	...
	{
		if ($str == '') return TRUE;
		$row = $this->Member_model->count_by( array('mem_userid' => $str, 'mem_denied' => 0));

		if ($row == 0) {
			$this->form_validation->set_message('_mem_recommend_check', $str . ' 는 존재하지 않는 추천인 회원아이디입니다');
			return FALSE;
		}
		return TRUE;
	...
	*/
	public function _check_captcha($str)
	{
		...
		if ( ! is_array($captcha) OR ! element('word', $captcha) OR strtolower(element('word', $captcha)) != strtolower($str)) {
			...
			$this->form_validation->set_message('_check_captcha',  '자동등록방지코드가 잘못되었습니다');
			return FALSE;
		}
		return TRUE;
	}
	...
	*/
	public function _check_recaptcha($str)
	{
		...
		$data = array('secret' => $this->cbconfig->item('recaptcha_secret'), 'response' => $str);
		...
		if ( $obj->success != "1" ) { 
			$this->form_validation->set_message('_check_recaptcha',  '자동등록방지코드가 잘못되었습니다');
			return FALSE; 
		}
