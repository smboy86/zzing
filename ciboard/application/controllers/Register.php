...
class Register extends CB_Controller
{
    ...
    protected $models = array(
        'Member_nickname', 'Member_meta', 'Member_auth_email',
        'Member_register', 'Note', 'Member_extra_vars');
    ...
    public function form()
    {
        ...
        $configbasic['mem_userid'] = array(
            'field' => 'mem_userid',
            'label' => '아이디',
            'rules' => 'trim|required|alphanumunder|min_length[3]|max_length[20]|is_unique[member.mem_userid]|callback__mem_userid_check',
        ...
        if ($form_validation === false OR $file_error !== '' OR $file_error2 !== '') {
            ...
            if ($this->cbconfig->item('send_email_register_admin')
                OR $this->cbconfig->item('send_note_register_admin')) {
                $mselect = 'mem_id, mem_email, mem_nickname, mem_phone';
                $adminlist = $this->Member_model->get_superadmin_list($mselect);
            }
