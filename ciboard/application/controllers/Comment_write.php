...
class Comment_write extends CB_Controller
{
	...
	*/
	public function update()
	{
		...
		if ($mode == 'cu' && ! $this->input->post('cmt_id')) {
			$result = array('error' => '잘못된 접근입니다');
			exit(json_encode($result));
		}
		if ($mode == 'cu') {
		...
		if ($this->input->post('cmt_id') && $mode == 'c')  {
			$parent_id = $this->input->post('cmt_id');
		...
		if ($mode == 'cu') {
			...
			if (element('mem_id', $comment) && element('mem_id', $comment) != $this->member->item('mem_id')  && $is_admin == FALSE) {
		...
		*/
		$config = array(
			array('field'=>'cmt_content', 'label'=>'내용', 'rules'=>'trim|required'),
		);
		if ($is_comment_name) {
			...
			*/
			...
			if ($mode == 'c') {
				...
				if ($is_comment_name) {
					...
					$updatedata['cmt_nickname'] = $this->input->post('cmt_nickname');
					$updatedata['cmt_password'] = password_hash($this->input->post('cmt_password'), PASSWORD_BCRYPT);
				}

				if ($can_comment_secret) {
					$updatedata['cmt_secret'] = $this->input->post('cmt_secret');
				}
				...
				if (element('send_note_comment_comment_writer', $board) && $this->member->item('mem_use_note')) {
					$notesendlistcmtwriter['mem_id'] = $this->member->item('mem_id');
				}
				...
				$replaceconfig = array(
					...
					site_url(),
					display_html_content($this->input->post('cmt_content'), 0),
				...
				if ($emailsendlistadmin) {
					...
					$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_comment_admin_content'));
				...
				if ($emailsendlistpostwriter) {
					...
					$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_comment_post_writer_content'));
					$this->email->clear(TRUE);
					$this->email->from($this->cbconfig->item('webmaster_email'), $this->cbconfig->item('webmaster_name'));
				...
				if ($emailsendlistcmtwriter) {
					...
					$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_comment_comment_writer_content'));
					$this->email->clear(TRUE);
					$this->email->from($this->cbconfig->item('webmaster_email'), $this->cbconfig->item('webmaster_name'));
				...
				if ($notesendlistadmin) {
					...
					$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_comment_admin_content'));
				...
				if ($notesendlistpostwriter && element('mem_id', $notesendlistpostwriter)) {
					...
					$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_comment_post_writer_content'));
				...
				if ($notesendlistcmtwriter && element('mem_id', $notesendlistcmtwriter)) {
					...
					$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_comment_comment_writer_content'));
