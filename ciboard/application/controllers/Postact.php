...
class Postact extends CB_Controller
{
	...
	*/
	...
	function __construct()
	{
		...
		*  라이브러리를 로딩합니다
		*/
		$this->load->library(array('pagination', 'querystring', 'accesslevel', 'email', 'notelib', 'point'));
	}
	...
	public function delete($post_id ='')
	{
		...
		if ( ! $post_id OR ! is_numeric($post_id)) {
			show_404();
		}
		...
		if (element('mem_id', $post)) {
			if ($is_admin == FALSE AND $this->member->item('mem_id') != element('mem_id', $post)) {
		...
		if (element('use_post_delete_log', $board)) {
			$this->Post_model->update(element('post_id', $post), array('post_del' => 1));
			$metadata = array(
				'delete_mem_id' => $this->member->item('mem_id'),
	...
	public function multi_delete($flag='')
	{
		...
		foreach ($post_ids as $post_id) {
			if ( ! is_numeric($post_id) OR $post_id < 1) {
	...
	public function delete_comment()
	{
		...
		if ( ! $cmt_id OR ! is_numeric($cmt_id)) {
			$result = array('error' => '잘못된 접근입니다');
			exit(json_encode($result));
		}
	...
	public function comment_multi_delete()
	{
		...
		foreach ($cmt_ids as $cmt_id) {
			...
			if (element('error', $result)) exit($return);
		}
	...
	public function download($file_id = '')
	{
		...
		if ( ! is_numeric($file_id) OR $file_id < 1) {
			show_404();
		}
		...
		$alertmessage = $this->member->is_member() ? '회원님은 다운로드 할 수 있는 권한이 없습니다' : '비회원은 다운로드할 수 있는 권한이 없습니다.\\n\\n회원이시라면 로그인 후 이용해 보십시오';
		$check = array('group_id' => element('bgr_id', $board), 'board_id' => element('brd_id', $board));
		$this->accesslevel->check(element('access_download' , $board), element('access_download_level', $board), element('access_download_group', $board) , $alertmessage, $check);

		if (element('comment_to_download', $board) && $is_admin == FALSE && $this->member->item('mem_id') && $this->member->item('mem_id') != element('mem_id', $post)) {
			$where = array(
		...
		if (element('like_to_download', $board) && $is_admin == FALSE && $this->member->item('mem_id') && $this->member->item('mem_id') != element('mem_id', $post)) {
			$where = array(
		...
		if ($this->member->item('mem_id') != element('mem_id', $post) && element('use_point', $board)) {
			$point = $this->point->insert_point($this->member->item('mem_id'), element('point_filedownload', $board), element('board_name', $board) . ' ' . element('post_id', $file) . ' 파일 다운로드', 'filedownload' , $file_id, '파일 다운로드');
			if (element('point_filedownload', $board) < 0 && $point < 0 && $this->cbconfig->item('block_download_zeropoint')) {
				$this->point->delete_point($this->member->item('mem_id'), 'filedownload' , $file_id, '파일 다운로드');
				alert('회원님은 포인트가 부족하므로 다운로드하실 수 없습니다. 다운로드시 ' . (element('point_filedownload', $board) * -1) . ' 포인트가 차감됩니다');
				return FALSE;
			}
			$point = $this->point->insert_point(element('mem_id', $post), element('point_filedownload_uploader', $board), element('board_name', $board) . ' ' . element('post_id', $file) . ' 파일 다운로드', 'file_uploader' , $file_id, '파일 다운로드 업로더 - ' . $this->member->item('mem_id'));
		}
		...
		if ( ! $this->session->userdata('post_file_download_' . element('pfi_id', $file))) {
			...
			$this->session->set_userdata('post_file_download_' . element('pfi_id', $file), '1');
		...
		$data = file_get_contents('uploads/post/' . element('pfi_filename', $file)); // Read the file's contents
	...
	public function link($link_id = '')
	{
		...
		if ( ! $link_id OR ! is_numeric($link_id)) {
			show_404();
		}
		if ( ! is_numeric($link_id) OR $link_id < 1) {
			show_404();
		}
		...
		if ( ! $this->session->userdata('post_link_click_' . element('pln_id', $link))) {
			...
			$this->session->set_userdata('post_link_click_' . element('pln_id', $link), '1');
	 ...
	 * 게시물 추천/비추천 하기
	 */
	public function post_like($post_id='' , $like_type='')
	{
		...
		if ( ! is_numeric($like_type) OR !($like_type == 1 OR $like_type == 2)) {
			$result = array('error' => '잘못된 접근입니다');
			exit(json_encode($result));
		}
		if ( ! $this->session->userdata('post_id_' . $post_id)) {
		...
		$post = $this->Post_model->get_one($post_id, 'post_id, brd_id, mem_id, post_del');
		...
		if (element('mem_id', $post) == $this->member->item('mem_id')) {
		...
		$exist = $this->Like_model->get_one('', 'lik_id, lik_type', $where);
		...
		$insertdata = array(
			...
			'mem_id' => $this->member->item('mem_id'),
			'target_mem_id' => element('mem_id', $post),
			'lik_type' => $like_type,
		...
		if (element('use_point', $board)) {
			if ($like_type == '1') {
				$field = 'post_like';
				$point = $this->point->insert_point($this->member->item('mem_id'), element('point_post_like', $board), element('board_name', $board) . ' ' . $post_id . ' 추천', 'like' , $post_id, '추천');
				$point = $this->point->insert_point(element('mem_id', $post), element('point_post_liked', $board), element('board_name', $board) . ' ' . $post_id . ' 추천받음', 'liked' , $post_id, '추천받음');
			}
			if ($like_type == '2') {
				$field = 'post_dislike';
				$point = $this->point->insert_point($this->member->item('mem_id'), element('point_post_dislike', $board), element('board_name', $board) . ' ' . $post_id . ' 추천', 'like' , $post_id, '추천');
				$point = $this->point->insert_point(element('mem_id', $post), element('point_post_disliked', $board), element('board_name', $board) . ' ' . $post_id . ' 비추천받음', 'disliked' , $post_id, '비추천받음');
			}
		}
		...
		$this->Post_model->update($post_id, array($field => $count));
	 ...
	 * 댓글 추천/비추천 하기
	 */
	public function comment_like($cmt_id='' , $like_type='')
	{
		...
		if ( ! is_numeric($cmt_id) OR $cmt_id < 1) {
			$result = array('error' => '잘못된 접근입니다');
			exit(json_encode($result));
		}
		if ( ! is_numeric($like_type) OR !($like_type == 1 OR $like_type == 2)) {
		...
		$comment = $this->Comment_model->get_one($cmt_id, 'cmt_id, post_id, mem_id, cmt_del');
		...
		$post = $this->Post_model->get_one(element('post_id', $comment), 'post_id, brd_id, mem_id, post_del');
		...
		if (element('mem_id', $comment) == $this->member->item('mem_id')) {
			$result = array('error' => '본인의 글에는 추천/비추천 기능을 사용할 수 없습니다');
			exit(json_encode($result));
		}
		...
		$insertdata = array(
			...
			'brd_id' => element('brd_id', $post),
			'mem_id' => $this->member->item('mem_id'),
			'target_mem_id' => element('mem_id', $comment),
			'lik_type' => $like_type,
			'lik_datetime' => cdate('Y-m-d H:i:s'),
			'lik_ip' => $this->input->ip_address(),
		);
		...
		if (element('use_point', $board)) {
			if ($like_type == '1') {
				$point = $this->point->insert_point($this->member->item('mem_id'), element('point_comment_like', $board), element('board_name', $board) . ' ' . $cmt_id . ' 추천', 'comment_like' , $cmt_id, '추천');
				$point = $this->point->insert_point(element('mem_id', $comment), element('point_comment_liked', $board), element('board_name', $board) . ' ' . $cmt_id . ' 추천받음', 'comment_liked' , $cmt_id, '추천받음');
			}
			if ($like_type == '2') {
				$point = $this->point->insert_point($this->member->item('mem_id'), element('point_comment_dislike', $board), element('board_name', $board) . ' ' . $cmt_id . ' 추천', 'comment_like' , $cmt_id, '추천');
				$point = $this->point->insert_point(element('mem_id', $comment), element('point_comment_disliked', $board), element('board_name', $board) . ' ' . $cmt_id . ' 비추천받음', 'disliked' , $cmt_id, '비추천받음');
			}
		}
		...
		$this->Comment_model->update($cmt_id, array($field => $count));
	 ...
	 * 게시물 스크랩 하기
	 */
	public function post_scrap($post_id='')
	{
	 ...
	 * 게시물 신고 하기
	 */
	public function post_blame($post_id='')
	{
		...
		$check = array('group_id' => element('bgr_id', $board), 'board_id' => element('brd_id', $board));
		$can_blame = $this->accesslevel->is_accessable(element('access_blame' , $board), element('access_blame_level', $board), element('access_blame_group', $board) , $check);
		...
		$notesendlistpostwriter = array();
		 ...
		 $writer = $this->Member_model->get_one(element('mem_id', $post));
		
		if (element('send_email_blame_super_admin', $board) OR element('send_note_blame_super_admin', $board)) {
			$superadminlist = $this->Member_model->get('', 'mem_id, mem_email, mem_nickname, mem_phone', array('mem_is_admin' => 1, 'mem_denied' => 0));
		}
		if (element('send_email_blame_group_admin', $board) OR element('send_note_blame_group_admin', $board)) {
		...
		if (element('send_email_blame_board_admin', $board) OR element('send_note_blame_board_admin', $board)) {
		...
		if (element('send_email_blame_group_admin', $board) && $groupadminlist) {
			foreach ($groupadminlist as $key => $value) { $emailsendlistadmin[$value['mem_id']] = $value; }
		}
		if (element('send_email_blame_board_admin', $board) && $boardadminlist) {
			foreach ($boardadminlist as $key => $value) { $emailsendlistadmin[$value['mem_id']] = $value; }
		}
		if (element('send_email_blame_post_writer', $board) && element('mem_receive_email', $writer)) {
		...
		if (element('send_note_blame_group_admin', $board) && $groupadminlist) {
			foreach ($groupadminlist as $key => $value) { $notesendlistadmin[$value['mem_id']] = $value; }
		}
		if (element('send_note_blame_board_admin', $board) && $boardadminlist) {
			foreach ($boardadminlist as $key => $value) { $notesendlistadmin[$value['mem_id']] = $value; }
		}
		...
		if ($emailsendlistadmin) {
			$title = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_blame_admin_title'));
			$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_blame_admin_content'));
		...
		if ($emailsendlistpostwriter) {
			$title = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_blame_post_writer_title'));
			$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_blame_post_writer_content'));
			$this->email->clear(TRUE);
			$this->email->from($this->cbconfig->item('webmaster_email'), $this->cbconfig->item('webmaster_name'));
		...
		if ($notesendlistadmin) {
			$title = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_blame_admin_title'));
			$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_blame_admin_content'));
		...
		if ($notesendlistpostwriter && element('mem_id', $notesendlistpostwriter)) {
			$title = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_blame_post_writer_title'));
			$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_blame_post_writer_content'));
		...
		$result = array('success' => '이 글을 신고 하셨습니다', 'count' => $count);
		exit(json_encode($result));
	 ...
	 * 댓글 신고 하기
	 */
	public function comment_blame($cmt_id='')
	{
		...
		$this->load->model(array('Comment_model', 'Blame_model'));
		...
		$comment_writer = $this->Member_model->get_one(element('mem_id', $comment));


		if (element('send_email_comment_blame_super_admin', $board) OR element('send_note_comment_blame_super_admin', $board)) {
			$superadminlist = $this->Member_model->get('', 'mem_id, mem_email, mem_nickname,mem_phone', array('mem_is_admin' => 1, 'mem_denied' => 0));
		}
		if (element('send_email_comment_blame_group_admin', $board) OR element('send_note_comment_blame_group_admin', $board)) {
			$this->load->model('Board_group_admin_model');
			$groupadminlist = $this->Board_group_admin_model->get_board_group_admin_member(element('bgr_id', $board));
		}
		if (element('send_email_comment_blame_board_admin', $board) OR element('send_note_comment_blame_board_admin', $board)) {
			$this->load->model('Board_admin_model');
			$boardadminlist = $this->Board_admin_model->get_board_admin_member(element('brd_id', $board));
		}
		...
		if((element('mem_email', $post_writer) && element('post_receive_email', $post)) OR (element('send_email_comment_blame_post_writer', $board) && element('mem_receive_email', $post_writer))) {
		...
		if(element('send_note_comment_blame_post_writer', $board) && element('mem_use_note', $post_writer)) {
			$notesendlistpostwriter['mem_id'] = element('mem_id', $post_writer);
		}
		if (element('send_note_comment_blame_comment_writer', $board) && element('mem_use_note', $comment_writer)) {
		...
		if ($emailsendlistadmin) {
			$title = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_comment_blame_admin_title'));
			$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_comment_blame_admin_content'));
			foreach ($emailsendlistadmin as $akey => $aval) {
				$this->email->clear(TRUE);
				$this->email->from($this->cbconfig->item('webmaster_email'), $this->cbconfig->item('webmaster_name'));
				$this->email->to(element('mem_email', $aval));
		...
		if ($emailsendlistpostwriter) {
			$title = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_comment_blame_post_writer_title'));
			$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_comment_blame_post_writer_content'));
			$this->email->clear(TRUE);
			$this->email->from($this->cbconfig->item('webmaster_email'), $this->cbconfig->item('webmaster_name'));
			$this->email->to(element('mem_email', $emailsendlistpostwriter));
			$this->email->subject($title);
		...
		if ($emailsendlistcmtwriter) {
			$title = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_comment_blame_comment_writer_title'));
			$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_comment_blame_comment_writer_content'));
			$this->email->clear(TRUE);
			$this->email->from($this->cbconfig->item('webmaster_email'), $this->cbconfig->item('webmaster_name'));
		...
		if ($notesendlistadmin) {
			$title = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_comment_blame_admin_title'));
			$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_comment_blame_admin_content'));
			foreach ($notesendlistadmin as $akey => $aval) {
				$note_result = $this->notelib->send_note($sender = 0, $receiver = element('mem_id', $aval), $title, $content, '1');
			}
		}
		if ($notesendlistpostwriter && element('mem_id', $notesendlistpostwriter)) {
			$title = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_comment_blame_post_writer_title'));
			$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_comment_blame_post_writer_content'));
			$note_result = $this->notelib->send_note($sender = 0, $receiver = element('mem_id', $notesendlistpostwriter), $title, $content, '1');
		}
		if ($notesendlistcmtwriter && element('mem_id', $notesendlistcmtwriter)) {
			$title = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_comment_blame_comment_writer_title'));
			$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_comment_blame_comment_writer_content'));
		...
		$result = array('success' => '이 댓글을 신고 하셨습니다', 'count' => $count);
		exit(json_encode($result));
	 ...
	 * 게시물 비밀글 설정 및 해제 하기
	 */
	public function post_secret($post_id='', $flag = '1')
	{
		...
		$post = $this->Post_model->get_one($post_id, 'post_id, brd_id, mem_id');
	 ...
	 */
	public function post_multi_secret($flag='')
	{
	 ...
	 * 댓글 비밀글 설정 및 해제 하기
	 */
	public function comment_secret($cmt_id='', $flag = '1')
	{
	 ...
	 */
	public function comment_multi_secret($flag = '1')
	{
	 ...
	 * 댓글 감춤/해제 하기
	 */
	public function post_hide_comment($post_id='', $flag = '1')
	{
	 ...
	 * 공지사항 올림/내림 설정
	 */
	public function post_notice($post_id='', $flag = '1')
	{
	 ...
	 */
	public function post_multi_notice($flag = '1')
	{
	 ...
	 * 게시물 블라인드 설정/해제하기
	 */
	public function post_blame_blind($post_id='', $flag = '1')
	{
		...
		$post = $this->Post_model->get_one($post_id, 'post_id, brd_id, mem_id, post_blame, post_del');
	 ...
	 */
	public function post_multi_blame_blind($flag = '1')
	{
	 ...
	 * 댓글 블라인드 설정/해제하기
	 */
	public function comment_blame_blind($cmt_id='', $flag = '1')
	{
		...
		$comment = $this->Comment_model->get_one($cmt_id, 'cmt_id, post_id, mem_id, cmt_blame, cmt_del');
		...
		$post = $this->Post_model->get_one(element('post_id', $comment), 'post_id, brd_id, mem_id, post_blame, post_del');
	 ...
	 */
	public function comment_multi_blame_blind($flag = '1')
	{
	...
	public function post_trash($post_id='')
	{
		...
		$this->Post_model->update($post_id, array('post_del' => 2));
		$metadata = array(
		...
		$cmts = $this->Comment_model->get('', 'cmt_id', array('post_id' => $post_id));
		if($cmts && is_array($cmts)) {
			foreach($cmts as $cmt) {
				$cmt_id = element('cmt_id', $cmt);
				$this->Comment_model->update($cmt_id, array('cmt_del' => 2));
				$metadata = array(
					'trash_mem_id' => $this->member->item('mem_id'),
					'trash_datetime' => cdate('Y-m-d H:i:s'),
					'trash_ip' => $this->input->ip_address(),
		...
		$result = array('success' => $success , 'url' => board_url(element('brd_key', $board)));
		exit(json_encode($result));
	 ...
	 * 댓글 휴지통으로 이동하기
	 */
	public function comment_trash($cmt_id='')
	{
		...
		$result = array('success' => $success , 'url' => post_url(element('brd_key', $board), element('post_id', $post)));
		exit(json_encode($result));
	...
	public function filter_spam_keyword()
	{
		...
		$return = array('title'=> $return_title, 'content' => $return_content);
	...
	public function tempsave()
	{
		...
		if ( ! $brd_id) exit();
		if ( ! $post_content or $post_content == '<p>&nbsp;</p>' or $post_content == '<div>&nbsp;</div>' or $post_content == '&nbsp;') exit();
		if ( ! $this->member->is_member()) exit();
	...
	public function get_tempsave()
	{
		...
		if ( ! $brd_id) return;
		if ( ! $this->member->is_member()) return;
	 ...
	 */
	public function naversyndi($post_id='')
	{
		...
		echo "<updated>" . cdate('Y-m-d\TH:i:s\+09:00') . "</updated>\n";

		echo "<link rel=\"site\" href=\"" . $site_url . "\" title=\"" . html_escape($this->cbconfig->item('site_title')) . "\" />\n";
		echo "<entry>\n";
			echo "<id>" . post_url(element('brd_key', $board), $post_id) . "</id>\n";
			echo "<title><![CDATA[" . html_escape(element('post_title', $post)) . "]]></title>\n";
			echo "<author>\n";
				echo "<name>" . html_escape(element('post_nickname', $post)) . "</name>\n";
			echo "</author>\n";
			echo "<updated>"  .date('Y-m-d\TH:i:s\+09:00', strtotime(element('post_datetime', $post))) . "</updated>\n";
			echo "<published>" . date('Y-m-d\TH:i:s\+09:00', strtotime(element('post_datetime', $post))) . "</published>\n";
			echo "<link rel=\"via\" href=\"" . board_url(element('brd_key', $board)) . "\" title=\"" . html_escape(element('brd_name', $board)) . "\" />\n";
			echo "<link rel=\"mobile\" href=\"" . post_url(element('brd_key', $board), $post_id) . "\" />\n";
			echo "<content type=\"html\"><![CDATA[{$content}]]></content>\n";
			echo "<summary type=\"text\"><![CDATA[{$summary}]]></summary>\n";
			echo "<category term=\"" . element('brd_key', $board) . "\" label=\"" . html_escape(element('brd_name', $board)) . "\" />\n";
		echo "</entry>\n";
		echo "</feed>";
