...
class Post extends CB_Controller
{
	...
	*  모델을 로딩합니다
	*/
	protected $models = array('Post', 'Member', 'Board', 'Comment', 'Blame', 'Like', 'Post_file', 'Post_link', 'Post_meta', 'Scrap', 'Comment_meta', 'Board_category');
	...
	public function listtrash()
	{
		...
		*/
		if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
			foreach ($this->input->post('chk') as $val) {
				if ($val) {
					$this->Post_model->update($val, array('post_del' => 2));
					$metadata = array(
		...
		* 삭제가 끝난 후 목록페이지로 이동합니다
		*/
		$this->session->set_flashdata('message', '정상적으로 휴지통으로 이동되었습니다');
		$param =& $this->querystring;
		$redirecturl = admin_url($this->pagedir . '?' .  $param->output());
		redirect($redirecturl);
