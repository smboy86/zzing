...
class Fileupload extends CB_Controller
{
	...
	*  모델을 로딩합니다
	*/
	protected $models = array('Post_file', 'Post', 'Member');
	...
	*/
	public function graph()
	{
		...
		$result = $this->{$this->modelname}->get_post_file_by_date($datetype, $start_date, $end_date, $brd_id);
		$sum_count = 0;
		$arr = array();
		$max = 0;
	...
	* 파일다운로드시 실행되는 함수입니다
	*/
	public function download($pfi_id = '')
	{
		...
		if ( ! is_numeric($pfi_id) OR $pfi_id < 1) {
			show_404();
		}
