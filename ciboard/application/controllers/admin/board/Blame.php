...
class Blame extends CB_Controller
{
	...
	*  모델을 로딩합니다
	*/
	protected $models = array('Blame', 'Board', 'Post', 'Comment', 'Member');
	...
	*/
	public function index()
	{
		...
		*/
		...
		$where = array();
		if($param->get('brd_id')) $where['blame.brd_id'] = $param->get('brd_id');
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				...
				if (element('target_type', $val) == 1) {
					...
					$result['list'][$key]['post'] = $post = $this->Post_model->get_one(element('target_id', $val), 'post_id, brd_id, post_title, post_datetime, post_ip');
					if ($post) {
					...
					$result['list'][$key]['comment'] = $comment = $this->Comment_model->get_one(element('target_id', $val), 'cmt_id, post_id, cmt_content, cmt_datetime, cmt_ip');
					if ($comment) {
						$result['list'][$key]['post'] = $post = $this->Post_model->get_one(element('post_id', $comment), 'post_id, brd_id, post_title, post_datetime, post_ip');
	...
	*/
	public function graph()
	{
		...
		$result = $this->{$this->modelname}->get_blame_count($datetype, $start_date, $end_date, $brd_id);
		$sum_count = 0;
		$arr = array();
		$max = 0;
		...
		if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
			foreach ($this->input->post('chk') as $val) {
				if ($val) {
					...
					if ( ! element('bla_id', $getdata)) continue;

					$this->{$this->modelname}->delete($val);

					$where = array('target_id' => element('target_id', $getdata), 'target_type' => element('target_type', $getdata));
					$blame_cnt = $this->{$this->modelname}->count_by($where);

					if (element('target_type', $getdata) == '1') { // 원글 신고의 경우
						$this->Post_model->update(element('target_id', $getdata) ,  array('post_blame' => $blame_cnt));
					} else if (element('target_type', $getdata) == '1') { // 댓글 신고의 경우
						$this->Comment_model->update(element('target_id', $getdata) ,  array('cmt_blame' => $blame_cnt));
					}
				}
			}
		}
