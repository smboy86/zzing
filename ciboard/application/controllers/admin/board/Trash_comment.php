...
class Trash_comment extends CB_Controller
{
	...
	*  모델을 로딩합니다
	*/
	protected $models = array('Post', 'Member', 'Comment', 'Board', 'Blame', 'Comment_meta');
	...
	*/
	public function index()
	{
		...
		*/
		...
		if($param->get('brd_id')) $where['post.brd_id'] = $param->get('brd_id');
	...
	public function listrecover()
	{
		...
		*/
		if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
			foreach ($this->input->post('chk') as $val) {
				if ($val) {
					$this->Comment_model->update($val, array('cmt_del' => 0));
	...
	*/
	public function truncate()
	{
		...
		$result = $this->Comment_model->get('', '', array('cmt_del' => 2));
	...
	* 뷰페이지에서 선택삭제를 하는 경우 실행되는 메소드입니다
	*/
	public function recover($cmt_id='')
	{
