...
class Editorimage extends CB_Controller
{
	...
	*  모델을 로딩합니다
	*/
	protected $models = array('Editor_image', 'Member');
	...
	*/
	public function graph()
	{
		...
		$result = $this->{$this->modelname}->get_editor_file_by_date($datetype, $start_date, $end_date);
		$sum_count = 0;
		$arr = array();
		$max = 0;
	...
	* 파일다운로드시 실행되는 함수입니다
	*/
	public function download($eim_id = '')
	{
		...
		if ( ! is_numeric($eim_id) OR $eim_id < 1) {
			show_404();
		}
		...
		$data = file_get_contents('uploads/editor/' . element('eim_filename', $file)); // Read the file's contents
