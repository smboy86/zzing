...
class Boardgroup extends CB_Controller
{
	...
	*  모델을 로딩합니다
	*/
	protected $models = array('Board_group', 'Board_group_meta', 'Board', 'Member', 'Board_group_admin');
	...
	*/
	public function index()
	{
		...
		*/
		...
		$findex = $param->get('findex', 'bgr_order');
		...
		*/
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				$result['list'][$key]['brd_cnt'] = $this->Board_model->count_by( array('bgr_id' => element('bgr_id', $val)));
				$result['list'][$key]['meta'] = $this->Board_group_meta_model->get_all_meta(element('bgr_id', $val));
				$result['list'][$key]['group_skin_option'] = get_skin_name('group', element('group_skin', $result['list'][$key]['meta']), '기본설정따름');
				$result['list'][$key]['group_mobile_skin_option'] = get_skin_name('group', element('group_mobile_skin', $result['list'][$key]['meta']), '기본설정따름');
				$result['list'][$key]['group_layout_option'] = get_skin_name('_layout', element('group_layout', $result['list'][$key]['meta']), '기본설정따름');
				$result['list'][$key]['group_mobile_layout_option'] = get_skin_name('_layout', element('group_mobile_layout', $result['list'][$key]['meta']), '기본설정따름');
	...
	public function write($pid = '')
	{
		...
		*/
		if ($pid) {
			...
			$getdata = array_merge($getdata, $this->Board_group_meta_model->get_all_meta($getdata['bgr_id']));
		}
		...
		*/
		$config = array(
			array('field'=>'bgr_name', 'label'=>'그룹명', 'rules'=>'trim|required'),
			array('field'=>'bgr_order', 'label'=>'그룹정렬순서', 'rules'=>'trim|required|numeric|is_natural'),
			array('field'=>'group_layout', 'label'=>'레이아웃', 'rules'=>'trim'),
			array('field'=>'group_mobile_layout', 'label'=>'모바일레이아웃', 'rules'=>'trim'),
			array('field'=>'group_sidebar', 'label'=>'사이드바', 'rules'=>'trim'),
			array('field'=>'group_mobile_sidebar', 'label'=>'모바일사이드바', 'rules'=>'trim'),
			array('field'=>'group_skin', 'label'=>'스킨', 'rules'=>'trim'),
			array('field'=>'group_mobile_skin', 'label'=>'모바일스킨', 'rules'=>'trim'),
		...
		*/
		if ($this->form_validation->run() == FALSE) {
			...
			*/
			...
			$updatedata = array(
				'bgr_key' => $this->input->post('bgr_key'),
	...
	*/
	public function write_admin($pid = '')
	{
		...
		*/
		$config = array(
			array('field'=>'bgr_id', 'label'=>'그룹아이디', 'rules'=>'trim|required|numeric|is_natural'),
		...
		*/
		$param =& $this->querystring;
		$findex = $param->get('findex', $this->Board_group_admin_model->primary_key);
		$forder = $param->get('forder', 'desc');

		$getdata = $this->{$this->modelname}->get_one($pid);
		$result = $this->Board_group_admin_model->get('', '', array('bgr_id' => $pid) , '' , '', $findex, $forder);
	...
	*/
	public function _userid_check($str='', $pid)
	{
		...
		if ( ! element('mem_id', $getdata)) {
			...
			$chkdata = $this->Board_group_admin_model->get_one('', '', array('bgr_id' => $pid, 'mem_id' => element('mem_id', $getdata)));
			...
			foreach ($this->input->post('chk') as $val) {
				if ($val) {
					...
					$updatedata = array(
						...
						'bgr_order' => element($val, $bgr_order),
					);
					$this->{$this->modelname}->update($val, $updatedata);
					$metadata = array(
						'group_layout' => $group_layout[$val],
						'group_mobile_layout' => $group_mobile_layout[$val],
						'group_sidebar' => $group_sidebar[$val],
						'group_mobile_sidebar' => $group_mobile_sidebar[$val],
						'group_skin' => $group_skin[$val],
						'group_mobile_skin' => $group_mobile_skin[$val],
					);
	...
	public function listdelete()
	{
		...
		*/
		if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
			foreach ($this->input->post('chk') as $val) {
				if ($val) {
					$cnt = $this->Board_model->count_by(array('bgr_id' => $val));
					if ($cnt) {
						$this->session->set_flashdata('message', '게시판이 존재하는 그룹은 삭제할 수 없습니다');
						$msg = 1;
					}
					if ( ! $cnt) {
						$this->{$this->modelname}->delete($val);
						$this->Board_group_admin_model->delete('', array('bgr_id' => $val));
