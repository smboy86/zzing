...
class Banner extends CB_Controller
{
		...
		*/
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				...
				if(empty($val['ban_start_date']) OR $val['ban_start_date'] == '0000-00-00') $result['list'][$key]['ban_start_date'] = '미지정';
				if(empty($val['ban_end_date']) OR $val['ban_end_date'] == '0000-00-00') $result['list'][$key]['ban_end_date'] = '미지정';
	...
	*/
	public function write($pid = '')
	{
		...
		*/
		$config = array(
			array('field'=>'bng_name', 'label'=>'배너위치', 'rules'=>'trim|required'),
			array('field'=>'ban_start_date', 'label'=>'배너시작일', 'rules'=>'trim|alpha_dash|exact_length[10]'),
			array('field'=>'ban_end_date', 'label'=>'배너종료일', 'rules'=>'trim|alpha_dash|exact_length[10]'),
			array('field'=>'ban_title', 'label'=>'이미지 설명', 'rules'=>'trim|required'),
			array('field'=>'ban_url', 'label'=>'이미지 URL', 'rules'=>'trim|valid_url'),
			array('field'=>'ban_target', 'label'=>'타겟', 'rules'=>'trim'),
			array('field'=>'ban_device', 'label'=>'배너표시기기', 'rules'=>'trim|required'),
			array('field'=>'ban_width', 'label'=>'이미지 가로값', 'rules'=>'trim|required|numeric|is_natural'),
			array('field'=>'ban_height', 'label'=>'이미지 세로값', 'rules'=>'trim|required|numeric|is_natural'),
			array('field'=>'ban_order', 'label'=>'정렬순서', 'rules'=>'trim|required|numeric|is_natural'),
			array('field'=>'ban_activated', 'label'=>'배너활성화', 'rules'=>'trim'),
		);
		...
		*/
		if ($form_validation == FALSE OR $file_error != '') {
			...
			if ($pid) {
				if(empty($getdata['ban_start_date']) OR $getdata['ban_start_date'] == '0000-00-00') $getdata['ban_start_date'] = '';
				if(empty($getdata['ban_end_date']) OR $getdata['ban_end_date'] == '0000-00-00') $getdata['ban_end_date'] = '';
			...
			*/
			...
			$updatedata = array(
				...
				'bng_name' => $this->input->post('bng_name'),
				'ban_url' => $this->input->post('ban_url'),
				'ban_target' => $this->input->post('ban_target'),
				'ban_device' => $this->input->post('ban_device'),
				'ban_width' => $this->input->post('ban_width'),
				'ban_height' => $this->input->post('ban_height'),
				'ban_order' => $this->input->post('ban_order'),
				'ban_activated' => $this->input->post('ban_activated'),
			);
			...
			if (element('ban_image', $getdata) && ($this->input->post('ban_image_del') OR $updatephoto)) {
				// 기존 파일 삭제
				 @unlink('./uploads/banner/' . element('ban_image', $getdata));
			}
			...
			*/
			if ($this->input->post($primary_key)) {
				$this->cache->delete('banner-' . element('bng_name', $getdata) . '-random-' . cdate('Y-m-d'));
				$this->cache->delete('banner-' . element('bng_name', $getdata) . '-order-' . cdate('Y-m-d'));
	...
	*/
	public function group()
	{
		...
		*/
		$config = array(
			array('field'=>'bng_name', 'label'=>'위치명', 'rules'=>'trim|required|alpha_dash|is_unique[banner_group.bng_name]'),
		);
		$this->form_validation->set_rules($config);
		...
		*/
		$param =& $this->querystring;
		$findex = $param->get('findex', 'bng_group');
		$forder = $param->get('forder', 'asc');

		$result = $this->Banner_group_model->get_admin_list(1000, 0, '' , '' , $findex, $forder);
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				$result['list'][$key]['banner_count'] = $this->Banner_model->count_by(array('bng_name' => element('bng_name', $val)));
			}
		}
