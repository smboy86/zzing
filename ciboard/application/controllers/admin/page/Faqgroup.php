...
class Faqgroup extends CB_Controller
{
	...
	*  모델을 로딩합니다
	*/
	protected $models = array('Faq', 'Faq_group',  'Member');
		...
		*/
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				...
				$result['list'][$key]['faqcount'] = $this->Faq_model->count_by( array('fgr_id' => element('fgr_id', $val)));
	...
	*/
	public function write($pid = '')
	{
		...
		*/
		$config = array(
			array('field'=>'fgr_title', 'label'=>'제목', 'rules'=>'trim|required'),
			array('field'=>'fgr_layout', 'label'=>'레이아웃', 'rules'=>'trim'),
			array('field'=>'fgr_sidebar', 'label'=>'사이드바 사용', 'rules'=>'trim'),
			array('field'=>'fgr_skin', 'label'=>'스킨', 'rules'=>'trim'),
			array('field'=>'fgr_mobile_layout', 'label'=>'모바일레이아웃', 'rules'=>'trim'),
			array('field'=>'fgr_mobile_sidebar', 'label'=>'모바일사이드바사용', 'rules'=>'trim'),
			array('field'=>'fgr_mobile_skin', 'label'=>'모바일스킨', 'rules'=>'trim'),
		);
		if ($this->input->post($primary_key)) {
			$config[] = array('field'=>'fgr_key', 'label'=>'FAQ 주소', 'rules'=>'trim|required|alpha_dash|min_length[3]|max_length[50]|is_unique[faq_group.fgr_key.fgr_id.' . $getdata['fgr_id'] . ']');
		} else {
			$config[] = array('field'=>'fgr_key', 'label'=>'FAQ 주소', 'rules'=>'trim|required|alpha_dash|min_length[3]|max_length[50]|is_unique[faq_group.fgr_key]');
		}
		...
		*/
		if ($this->form_validation->run() == FALSE) {
			...
			$view['view']['data']['fgr_layout_option'] = get_skin_name('_layout', set_value('fgr_layout', element('fgr_layout', $getdata)), '기본설정따름');
			$view['view']['data']['fgr_mobile_layout_option'] = get_skin_name('_layout', set_value('fgr_mobile_layout', element('fgr_mobile_layout', $getdata)), '기본설정따름');
			$view['view']['data']['fgr_skin_option'] = get_skin_name('faq', set_value('fgr_skin', element('fgr_skin', $getdata)), '기본설정따름');
			$view['view']['data']['fgr_mobile_skin_option'] = get_skin_name('faq', set_value('fgr_mobile_skin', element('fgr_mobile_skin', $getdata)), '기본설정따름');
			...
			*/
			...
			$updatedata = array(
				'fgr_title' => $this->input->post('fgr_title'),
				'fgr_key' => $this->input->post('fgr_key'),
				'fgr_layout' => $this->input->post('fgr_layout'),
				'fgr_mobile_layout' => $this->input->post('fgr_mobile_layout'),
				'fgr_sidebar' => $this->input->post('fgr_sidebar'),
				'fgr_mobile_sidebar' => $this->input->post('fgr_mobile_sidebar'),
				'fgr_skin' => $this->input->post('fgr_skin'),
				'fgr_mobile_skin' => $this->input->post('fgr_mobile_skin'),
			);
		...
		if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
			foreach ($this->input->post('chk') as $val) {
				if ($val) {
					...
					$this->Faq_model->delete('', array('fgr_id' => $val));
				}
			}
		}
