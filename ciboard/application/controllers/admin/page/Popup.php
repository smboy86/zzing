...
class Popup extends CB_Controller
{
	...
	*  모델을 로딩합니다
	*/
	protected $models = array('Popup', 'Member');
		...
		*/
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				if(empty($val['pop_start_date']) OR $val['pop_start_date'] == '0000-00-00') $result['list'][$key]['pop_start_date'] = '미지정';
				if(empty($val['pop_end_date']) OR $val['pop_end_date'] == '0000-00-00') $result['list'][$key]['pop_end_date'] = '미지정';
	...
	*/
	public function write($pid = '')
	{
		...
		*/
		$config = array(
			array('field'=>'pop_title', 'label'=>'팝업명', 'rules'=>'trim|required'),
			array('field'=>'pop_start_date', 'label'=>'팝업시작일', 'rules'=>'trim|alpha_dash|exact_length[10]'),
			array('field'=>'pop_end_date', 'label'=>'팝업종료일', 'rules'=>'trim|alpha_dash|exact_length[10]'),
			array('field'=>'pop_is_center', 'label'=>'팝업정렬', 'rules'=>'trim|required|numeric'),
			array('field'=>'pop_left', 'label'=>'좌측위치', 'rules'=>'trim|numeric'),
			array('field'=>'pop_top', 'label'=>'상단위치', 'rules'=>'trim|required|numeric'),
			array('field'=>'pop_width', 'label'=>'팝업가로길이', 'rules'=>'trim|required|numeric'),
			array('field'=>'pop_height', 'label'=>'팝업세로길이', 'rules'=>'trim|required|numeric'),
			array('field'=>'pop_device', 'label'=>'팝업표시기기', 'rules'=>'trim|required'),
			array('field'=>'pop_page', 'label'=>'팝업이뜨는페이지', 'rules'=>'trim|required|numeric'),
			array('field'=>'pop_disable_hours', 'label'=>'시간', 'rules'=>'trim|required|numeric'),
			array('field'=>'pop_activated', 'label'=>'팝업활성화', 'rules'=>'trim|required|numeric'),
			array('field'=>'pop_content', 'label'=>'팝업내용', 'rules'=>'trim|required'),
		);
		...
		*/
		if ($this->form_validation->run() == FALSE) {
			...
			if ($pid) {
				if(empty($getdata['pop_start_date']) OR $getdata['pop_start_date'] == '0000-00-00') $getdata['pop_start_date'] = '';
				if(empty($getdata['pop_end_date']) OR $getdata['pop_end_date'] == '0000-00-00') $getdata['pop_end_date'] = '';
			...
			*/
			...
			$updatedata = array(
				...
				'pop_start_date' => $this->input->post('pop_start_date'),
				'pop_end_date' => $this->input->post('pop_end_date'),
				'pop_is_center' => $this->input->post('pop_is_center'),
				'pop_left' => $this->input->post('pop_left'),
				'pop_top' => $this->input->post('pop_top'),
				'pop_width' => $this->input->post('pop_width'),
				'pop_height' => $this->input->post('pop_height'),
				'pop_device' => $this->input->post('pop_device'),
				'pop_page' => $this->input->post('pop_page'),
				'pop_disable_hours' => $this->input->post('pop_disable_hours'),
				'pop_activated' => $this->input->post('pop_activated'),
				'pop_content' => $this->input->post('pop_content'),
