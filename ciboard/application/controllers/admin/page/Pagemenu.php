...
class Pagemenu extends CB_Controller
{
	...
	*/
	public function index()
	{
		...
		*/
		$config = array(
			array('field'=>'men_name', 'label'=>'메뉴명', 'rules'=>'trim|required'),
			array('field'=>'men_parent', 'label'=>'메뉴위치', 'rules'=>'trim'),
			array('field'=>'men_target', 'label'=>'새창여부', 'rules'=>'trim'),
			array('field'=>'men_custom', 'label'=>'커스텀', 'rules'=>'trim'),
			array('field'=>'men_order', 'label'=>'순서', 'rules'=>'trim|numeric|is_natural'),
			array('field'=>'men_desktop', 'label'=>'PC사용', 'rules'=>'trim'),
			array('field'=>'men_mobile', 'label'=>'모바일사용', 'rules'=>'trim'),
			array('field'=>'men_link', 'label'=>'링크주소', 'rules'=>'trim|required|prep_url|valid_url'),
		);
		...
		*/
		if ($this->form_validation->run() == FALSE) {
			...
			*/
			...
			$updatedata = array(
				'men_parent' => $this->input->post('men_parent'),
		...
		*/
		...
		$result = $this->{$this->modelname}->get_admin_list($per_page = 1000, '', $where , '' , $findex = 'men_order', $forder = 'ASC', '', '');
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				$subwhere = array('men_parent' => element('men_id', $val));
				$subresult = $this->{$this->modelname}->get_admin_list($per_page = 1000, '', $subwhere , '' , $findex = 'men_order', $forder = 'ASC', '', '');
		...
		if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
			foreach ($this->input->post('chk') as $val) {
				if ($val) {
					...
					$men_link = $this->input->post('men_link');
					$updatedata = array(
						'men_name' => $men_name[$val],
						'men_target' => $men_target[$val],
						'men_custom' => $men_custom[$val],
						'men_order' => $men_order[$val],
						'men_desktop' => $men_desktop[$val],
						'men_mobile' => $men_mobile[$val],
						'men_link' => $men_link[$val],
					);
