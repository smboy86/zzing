...
class Cleanlog extends CB_Controller
{
	...
	*  모델을 로딩합니다
	*/
	protected $models = array('Member_login_log', 'Stat_count', 'Stat_count_date', 'Note', 'Autologin');
	...
	*/
	public function cache()
	{
		...
		*/
		$config = array(
			array('field'=>'execute', 'label'=>'실행', 'rules'=>'trim'),
		);
		...
		*/
		if ($this->form_validation->run() == FALSE) {
			...
			*/
			...
			$result = delete_files('./uploads/cache/', TRUE);
			if ($result !== FALSE) {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			*/
			...
			if ($this->input->post('criterion') && $this->input->post('day')) {
				$this->Stat_count_model->delete('', array('sco_date <=' => $this->input->post('criterion')));
				$this->Stat_count_date_model->delete('', array('scd_date <=' => $this->input->post('criterion')));
				$view['view']['alert_message'] = '총 ' . number_format($this->input->post('log_count')) . ' 건의 ' . $this->input->post('day') . '일 이상된 방문자로그가 모두 삭제되었습니다';
			} else {
				$criterion = cdate('Y-m-d', ctimestamp() - $this->input->post('day') * 24 * 60 * 60);
				$log_count = $this->Stat_count_model->count_by( array('sco_date <=' => $criterion));
				$view['view']['criterion'] = $criterion;
				$view['view']['day'] = $this->input->post('day');
				$view['view']['log_count'] = $log_count;
				if ($log_count > 0) {
					$view['view']['msg'] = '총 ' . number_format($log_count) . ' 건의 ' . $this->input->post('day') . '일 이상된 방문자로그가 발견되었습니다. 이를 모두 삭제하시겠습니까?';
				} else {
		...
		*/
		if ($this->form_validation->run() == FALSE) {
			...
			*/
			...
			if ($this->input->post('criterion') && $this->input->post('day')) {
				$this->Member_login_log_model->delete('', array('mll_datetime <=' => $this->input->post('criterion')));
				...
				$log_count = $this->Member_login_log_model->count_by( array('mll_datetime <=' => $criterion));
				$view['view']['criterion'] = $criterion;
				$view['view']['day'] = $this->input->post('day');
				$view['view']['log_count'] = $log_count;
				if ($log_count > 0) {
		...
		*/
		if ($this->form_validation->run() == FALSE) {
			...
			*/
			...
			if ($this->input->post('criterion') && $this->input->post('day')) {
				$this->Note_model->delete('', array('nte_datetime <=' => $this->input->post('criterion')));
				$view['view']['alert_message'] = '총 ' . number_format($this->input->post('log_count')) . ' 건의 ' . $this->input->post('day') . '일 이상된 쪽지가 모두 삭제되었습니다';
			} else {
				$criterion = cdate('Y-m-d H:i:s', ctimestamp() - $this->input->post('day') * 24 * 60 * 60);
				$log_count = $this->Note_model->count_by( array('nte_datetime <=' => $criterion));
				$view['view']['criterion'] = $criterion;
				$view['view']['day'] = $this->input->post('day');
				$view['view']['log_count'] = $log_count;
				if ($log_count > 0) {
		...
		*/
		if ($this->form_validation->run() == FALSE) {
			...
			*/
			...
			if ($this->input->post('criterion') && $this->input->post('day')) {
				$this->Autologin_model->delete('', array('aul_datetime <=' => $this->input->post('criterion')));
				$view['view']['alert_message'] = '총 ' . number_format($this->input->post('log_count')) . ' 건의 ' . $this->input->post('day') . '일 이상된 자동로그인로그가 모두 삭제되었습니다';
			} else {
				$criterion = cdate('Y-m-d H:i:s', ctimestamp() - $this->input->post('day') * 24 * 60 * 60);
				$log_count = $this->Autologin_model->count_by( array('aul_datetime <=' => $criterion));
				$view['view']['criterion'] = $criterion;
				$view['view']['day'] = $this->input->post('day');
				$view['view']['log_count'] = $log_count;
				if ($log_count > 0) {
