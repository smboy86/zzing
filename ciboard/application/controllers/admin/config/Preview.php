	...
	*/
	public function preview($pagetype)
	{
		...
		if($pagetype == 'board') {
			...
			if( ! $val) alert_close('최소 1개 이상의 게시물이 작성된 후에 미리보기가 가능합니다');
		...
		if($pagetype == 'group') {
			...
			if( ! $val) alert_close('최소 1개 이상의 그룹이 생성된 후에 미리보기가 가능합니다');
		...
		if($pagetype == 'document') {
			...
			$val = element(0, $latest);
			if( ! $val) alert_close('최소 1개 이상의 일반문서가 작성된 후에 미리보기가 가능합니다');
		...
		if($pagetype == 'faq') {
			...
			$val = element(0, $latest);
			if( ! $val) alert_close('최소 1개 이상의 FAQ가 작성된 후에 미리보기가 가능합니다');
		...
		if($pagetype == 'note') {
			...
			$val = element(0, $latest);
			if( ! $val) alert_close('최소 1개 이상의 쪽지를 수신한 후에 미리보기가 가능합니다');
		...
		$previewurl .=  '?layout=' . $this->input->get('layout') . '&sidebar=' . $this->input->get('sidebar') . '&skin=' . $this->input->get('skin') . '&is_mobile=' . $this->input->get('is_mobile');
		...
		$view['view']['layout_option'] = get_skin_name('_layout', $this->input->get('layout'), '기본설정따름');
		$view['view']['skin_option'] = get_skin_name($pagetype, $this->input->get('skin'), '기본설정따름');
		$view['view']['mobile_layout_option'] = get_skin_name('_layout', $this->input->get('mobile_layout'), '기본설정따름');
		$view['view']['mobile_skin_option'] = get_skin_name($pagetype, $this->input->get('mobile_skin'), '기본설정따름');
