...
class Loginlog extends CB_Controller
{
	...
	*  모델을 로딩합니다
	*/
	protected $models = array('Member_login_log', 'Member');
	...
	*/
	public function index()
	{
		...
		*/
		$this->{$this->modelname}->allow_search_field = array('mll_id', 'mll_success', 'member_login_log.mem_id', 'mll_datetime', 'mll_ip', 'mll_reason', 'mll_log_agent', 'mll_url', 'mll_referer'); // 검색이 가능한 필드
	...
	*/
	public function graph()
	{
		...
		$result = $this->{$this->modelname}->get_login_success_count($datetype, $start_date, $end_date);
		$sum_count = 0;
		$arr = array();
		$max = 0;
	...
	*/
	public function graph_fail()
	{
		...
		$result = $this->{$this->modelname}->get_login_fail_count($datetype, $start_date, $end_date);
		$sum_count = 0;
		$arr = array();
		$max = 0;
		if ($result && is_array($result)) {
