...
class Members extends CB_Controller
{
    ...
    public function write($pid = 0)
    {
        ...
        if ($this->input->post($primary_key)) {
            $config[] = array(
                'field' => 'mem_userid',
                'label' => '회원아이디',
                'rules' => 'trim|required|alphanumunder|min_length[3]|max_length[20]|is_unique[member.mem_userid.mem_id.' . element('mem_id', $getdata) . ']|callback__mem_userid_check',
            );
            $config[] = array(
                'field' => 'mem_password',
                'label' => '패스워드',
            ...
            $config[] = array(
                'field' => 'mem_userid',
                'label' => '회원아이디',
                'rules' => 'trim|required|alphanumunder|min_length[3]|max_length[20]|is_unique[member.mem_userid]',
            );
            $config[] = array(
                'field' => 'mem_password',
                'label' => '패스워드',
    ...
    public function listdelete()
    {
        ...
        $this->load->model(array(
            'Board_admin_model', 'Board_group_admin_model', 'Member_auth_email_model',
            'Member_extra_vars_model', 'Member_login_log_model', 'Member_meta_model',
            'Member_register_model', 'Notification_model', 'Point_model', 'Scrap_model'
        ));
        ...
        if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
            foreach ($this->input->post('chk') as $val) {
                if ($val) {
                    ...
                    $this->Board_group_admin_model->delete_where($where);
                    $this->Member_auth_email_model->delete_where($where);
                    $this->Member_extra_vars_model->delete_where($where);
                    $this->Member_login_log_model->delete_where($where);
                    $this->Member_meta_model->delete_where($where);
                    $this->Member_register_model->delete_where($where);
