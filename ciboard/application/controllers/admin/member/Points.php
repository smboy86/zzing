...
class Points extends CB_Controller
{
	...
	*  모델을 로딩합니다
	*/
	protected $models = array('Point', 'Member');
	...
	public function write()
	{
		...
		*/
		...
		if ($this->input->post('poi_type') == 'toall') {
			$config = array(
				array('field'=>'poi_point_all', 'label'=>'포인트', 'rules'=>'trim|required|numeric'),
				array('field'=>'poi_content_all', 'label'=>'내용', 'rules'=>'trim|required'),
			);
		} else if ($this->input->post('poi_type') == 'toone') {
			$config = array(
				array('field'=>'mem_userid', 'label'=>'회원아이디', 'rules'=>'trim|required|callback__real_userid'),
				array('field'=>'poi_point', 'label'=>'포인트', 'rules'=>'trim|required|numeric'),
				array('field'=>'poi_content', 'label'=>'내용', 'rules'=>'trim|required'),
			);
		}
		...
		*/
		if ($this->form_validation->run() == FALSE) {
			...
			*/
			...
			if ($this->input->post('poi_type') == 'toall') {
				$mball = $this->Member_model->get('' , 'mem_id', array('mem_denied' => 0));
				if ($mball && is_array($mball)) {
					foreach ($mball as $mb) {
						if (element('mem_id', $mb)) {
							$this->point->insert_point(element('mem_id', $mb), $this->input->post('poi_point_all'), $this->input->post('poi_content_all'), '@byadmin', element('mem_id', $mb), $this->member->item('mem_id') . '-' . uniqid(''));
						}
					}
				}
			} else if ($this->input->post('poi_type') == 'toone') {
				...
				if (element('mem_id', $mb)) {
					$this->point->insert_point(element('mem_id', $mb), $this->input->post('poi_point'), $this->input->post('poi_content'), '@byadmin', element('mem_id', $mb), $this->member->item('mem_id') . '-' . uniqid(''));
				}
			}
	...
	*/
	public function _real_userid($str)
	{
		...
		if ( ! element('mem_id', $mb)) {
			$this->form_validation->set_message('_real_userid', $str . ' 은 존재하지 않는 회원아이디입니다');
			return FALSE;
		}
		if (element('mem_denied', $mb)) {
			$this->form_validation->set_message('_real_userid', $str . ' 은 탈퇴, 차단된 회원아이디입니다');
			return FALSE;
		}
		return TRUE;
	}
