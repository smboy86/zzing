...
class Searchkeyword extends CB_Controller
{
	...
	*  모델을 로딩합니다
	*/
	protected $models = array('Search_keyword', 'Member');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			*/
			...
			if ($this->input->post('criterion') && $this->input->post('day')) {
				$this->Search_keyword_model->delete('', array('sek_datetime <=' => $this->input->post('criterion')));
				...
				$log_count = $this->Search_keyword_model->count_by( array('sek_datetime <=' => $criterion));
				$view['view']['criterion'] = $criterion;
				$view['view']['day'] = $this->input->post('day');
				$view['view']['log_count'] = $log_count;
				if ($log_count > 0) {
					$view['view']['msg'] = '총 ' . number_format($log_count) . ' 건의 ' . $this->input->post('day') . '일 이상된 검색어로그가 발견되었습니다. 이를 모두 삭제하시겠습니까?';
				} else {
