...
class Comment_list extends CB_Controller
{
	...
	*/
	...
	function __construct()
	{
		...
		*  라이브러리를 로딩합니다
		*/
		$this->load->library(array('pagination', 'querystring', 'accesslevel', 'email'));
	}
	...
	*/
	public function lists($post_id = '')
	{
		...
		if ( ! element('post_id', $post)) show_404();
		...
		*/
		...
		if ($this->cbconfig->get_device_view_type() == 'mobile') {
			$per_page = element('mobile_comment_count', $board) ? element('mobile_comment_count', $board) : 0;
		} else {
			$per_page = element('comment_count', $board) ? element('comment_count', $board) : 0;
		}
		...
		if ( ! is_numeric($page) OR $page < 1) show_404();
		...
		*/
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				...
				if (( ! $is_blind && ! element('cmt_secret', $val)) OR $is_admin OR (element('mem_id', $val) && element('mem_id', $val) == $this->member->item('mem_id')) OR (element('mem_id', $post) && element('mem_id', $post) == $this->member->item('mem_id'))) {
					...
					if (element('comment_syntax_highlighter', $board)) {
						$result['list'][$key]['content'] = preg_replace_callback("/(\[code\]|\[code=(.*)\])(.*)\[\/code\]/iUs", "content_syntaxhighlighter", $result['list'][$key]['content']); // SyntaxHighlighter
					}
				}
				...
				} else if (element('show_comment_ip', $board) == '1') {
				...
				if ( ! element('post_del', $post) && ! element('cmt_del', $val)) {
					...
					if ($is_admin OR (element('mem_id', $val) && $this->member->item('mem_id') == element('mem_id', $val))) {
					...
					if (strlen(element('cmt_reply', $val)) < 5 && $can_comment_write) {
		...
		*/
		...
		if ( ! $this->input->get('page')) $_GET['page'] = (string)$page;
		...
		*/
		...
		if ( ! $skindir) $skindir = ($this->cbconfig->get_device_view_type() == 'mobile') ? $this->cbconfig->item('mobile_skin_board') : $this->cbconfig->item('skin_board');
		if ( ! $skindir) $skindir = ($this->cbconfig->get_device_view_type() == 'mobile') ? $this->cbconfig->item('mobile_skin_default') : $this->cbconfig->item('skin_default');
		if ( ! $skindir) $skindir = 'basic';
	...
	*/
	public function password($post_id='', $cmt_id = '')
	{
		...
		if ( ! $cmt_id OR ! is_numeric($cmt_id)) show_404();
		if ( ! $post_id OR ! is_numeric($post_id)) show_404();

		$comment = $this->Comment_model->get_one($cmt_id);
		if ( ! element('cmt_id', $comment)) show_404();

		if (element('post_id', $comment) != $post_id) show_404();
		...
		*/
		$config = array(
			array('field'=>'modify_password', 'label'=>'패스워드', 'rules'=>'trim|required'),
		);
		...
		*/
		if ($this->form_validation->run() == FALSE) {
			...
			*/
			...
			$return = $this->board->delete_comment_check($cmt_id, $this->input->post('modify_password'));
			$result = json_decode($return, TRUE);

			if (element('error', $result)) $view['view']['message'] = element('error', $result);
			if (element('password', $result)) $view['view']['message'] = element('password', $result);
