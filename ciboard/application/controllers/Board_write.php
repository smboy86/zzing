...
class Board_write extends CB_Controller
{
	...
	*/
	public function write($brd_key='')
	{
		...
		$this->accesslevel->check(element('access_write' , $board), element('access_write_level', $board), element('access_write_group', $board) , $alertmessage, $check);
	...
	*  게시물 답변 페이지입니다
	*/
	public function reply($origin_id='')
	{
		...
		*/
		if ( ! $origin_id OR ! is_numeric($origin_id))  {
			show_404();
		}
		...
		$alertmessage = $this->member->is_member() ? '회원님은 답변할 수 있는 권한이 없습니다' : '비회원은 답변할 수 있는 권한이 없습니다.\\n\\n회원이시라면 로그인 후 이용해 보십시오';
		$check = array('group_id' => element('bgr_id', $board), 'board_id' => element('brd_id', $board));
		$this->accesslevel->check(element('access_reply' , $board), element('access_reply_level', $board), element('access_reply_group', $board) , $alertmessage, $check);
		...
		} else if (element('reply', $row) == $end_reply_char) { // A~Z은 26 입니다.
			alert('더 이상 답변하실 수 없습니다.\\n답변은 26개 까지만 가능합니다');
		} else {
	...
	*/
	public function _write_common($board, $origin='', $reply='')
	{
		...
		$view['view']['post']['can_post_notice'] = $can_post_notice = $is_admin ? TRUE : FALSE;
		...
		*/
		$config = array(
			array('field'=>'post_title', 'label'=>'제목', 'rules'=>'trim|required'),
			array('field'=>'post_content', 'label'=>'내용', 'rules'=>'trim|required'),
		);
		...
		if ($use_upload && $form_validation && element('use_upload_file', $board)) {
		...
		*/
		if ($form_validation == FALSE OR $file_error) {
			...
			*/
			...
			$mem_id = $this->member->item('mem_id') ? $this->member->item('mem_id') : '';
			if ($mem_id) {
			...
			if ($is_post_name) {
				$updatedata['post_nickname'] = $this->input->post('post_nickname');
				$updatedata['post_email'] = $this->input->post('post_email');
				$updatedata['post_homepage'] = $this->input->post('post_homepage');
			}
			...
			if ($can_post_notice) {
				$updatedata['post_notice'] = $this->input->post('post_notice');
			}
			if ($can_post_secret) {
				$updatedata['post_secret'] = $this->input->post('post_secret');
			}
			if (element('use_post_secret', $board) == 2) {
				$updatedata['post_secret'] = 1;
			}
			if ($can_post_receive_email) {
				$updatedata['post_receive_email'] = $this->input->post('post_receive_email');
			}
			if (element('use_category', $board)) {
				$updatedata['post_category'] = $this->input->post('post_category');
			}
			...
			if ($post_link && is_array($post_link) && count($post_link)>0) {
				...
				$this->Post_model->update($post_id , array('post_link_count' => count($post_link)));
			}
			...
			if ($this->member->is_member() && element('use_tempsave', $board)) {
				...
				$this->Tempsave_model->delete('', $tempwhere);
			}
			...
			if (element('send_note_post_writer', $board) && $this->member->item('mem_use_note')) {
				$notesendlistpostwriter['mem_id'] = $this->member->item('mem_id');
			}
			...
			$replaceconfig = array(
				...
				site_url(),
				html_escape($this->input->post('post_title')), 
			...
			if ($emailsendlistadmin) {
				...
				$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_post_admin_content'));
			...
			if ($emailsendlistpostwriter) {
				...
				$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_email_post_writer_content'));
				$this->email->clear(TRUE);
				$this->email->from($this->cbconfig->item('webmaster_email'), $this->cbconfig->item('webmaster_name'));
			...
			if ($notesendlistadmin) {
				...
				$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_post_admin_content'));
			...
			if ($notesendlistpostwriter && element('mem_id', $notesendlistpostwriter)) {
				...
				$content = str_replace($searchconfig, $replaceconfig, $this->cbconfig->item('send_note_post_writer_content'));
	...
	*/
	public function modify($post_id='')
	{
		...
		*/
		...
		$view['view']['post']['is_post_name'] = $is_post_name = ($this->member->is_member() == FALSE OR ($is_admin && $this->member->item('mem_id') != element('mem_id', $post))) ? TRUE : FALSE;
		$view['view']['post']['can_post_notice'] = $can_post_notice = $is_admin ? TRUE : FALSE;
		$view['view']['post']['can_post_secret'] = $can_post_secret = element('use_post_secret', $board) == '1' ? TRUE : FALSE;
		$view['view']['post']['can_post_receive_email'] = $can_post_receive_email = element('use_post_receive_email', $board) ? TRUE : FALSE;
		...
		*/
		$config = array(
			array('field'=>'post_id', 'label'=>'POSTID', 'rules'=>'trim|required|numeric'),
			array('field'=>'post_title', 'label'=>'제목', 'rules'=>'trim|required'),
			array('field'=>'post_content', 'label'=>'내용', 'rules'=>'trim|required'),
		);
		...
		if ($this->member->is_member() == FALSE) {
			$password_length = $this->cbconfig->item('password_length');
			$config[] = array('field'=>'post_password', 'label'=>'패스워드', 'rules'=>'trim|min_length[' . $password_length . ']|callback__mem_password_check');
			if($this->cbconfig->item('use_recaptcha')) {
				$config[] = array('field' => 'g-recaptcha-response', 'label' =>'자동등록방지문자', 'rules' => 'trim|required|callback__check_recaptcha');
			} else {
				$config[] = array('field' => 'captcha_key', 'label' =>'자동등록방지문자', 'rules' => 'trim|required|callback__check_captcha');
	...
	*/
	function _naver_syndi($post_id, $board)
	{
		if ( ! $this->cbconfig->item('naver_syndi_key')) return;

		// curl library 가 지원되어야 합니다.
		if (!function_exists('curl_init')) return;

		// 이 게시판은 신디케이션 기능을 사용하지 않습니다
		if ( ! element('use_naver_syndi', $board)) return;
		
		// 비회원이 읽기가 가능한 게시판만 신디케이션을 지원합니다
		if(element('access_view' , $board)) return;
		
		// 1:1 게시판은 신디케이션을 지원하지 않습니다
		if (element('use_personal', $board)) return;
