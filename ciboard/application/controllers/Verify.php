...
class Verify extends CB_Controller
{
	...
	*  모델을 로딩합니다
	*/
	protected $models = array('Member_auth_email', 'Member_login_log');
	...
	*/
	public function confirmemail()
	{
		...
		if ($this->member->item('mem_id')) redirect();
		...
		} else if ( ! (element('mae_type', $result) == '1' OR element('mae_type', $result) == '2')) {
			$view['view']['message'] = '잘못된 접근입니다';
		} else if ( ! empty($result['mae_use_datetime']) && element('mae_use_datetime', $result) != '0000-00-00 00:00:00') {
			$view['view']['message'] = '회원님은 이미 인증을 받으셨습니다';
		} else if ( strtotime(element('mae_generate_datetime', $result)) < ctimestamp()- 86400) {
			$view['view']['message'] = '24 시간 이내에 인증을 받으셔야 합니다';
		} else if (element('mae_expired', $result)) {
			$view['view']['message'] = '잘못된 접근입니다';
		} else {

			$dbmember = $this->Member_model->get_by_memid(element('mem_id', $result), 'mem_id, mem_userid, mem_denied, mem_email_cert');

			if ( ! element('mem_id', $dbmember)) {
				$view['view']['message'] = '잘못된 접근입니다';
			} else if (element('mem_userid', $dbmember) != $this->input->get('user')) {
				$view['view']['message'] = '잘못된 접근입니다';
			} else if (element('mem_denied', $dbmember)) {
				$view['view']['message'] = '탈퇴하신 아이디입니다';
			} else if (element('mem_email_cert', $dbmember)) {
				$view['view']['message'] = '회원님은 이미 인증을 받으셨습니다';
			} else {
				...
				$this->_update_login_log(element('mem_id', $dbmember), 1, '이메일 인증 후 로그인 성공');
	...
	*/
	public function resetpassword()
	{
		...
		if ( ! element('mae_id', $result)) {
			...
			$view['view']['message'] = '24 시간 이내에 인증을 받으셔야 합니다';
		} else if (element('mae_type', $result) != '3') {
			$view['view']['error_message'] = '잘못된 접근입니다';
		} else {
			$dbmember = $this->Member_model->get_by_memid(element('mem_id', $result), 'mem_id, mem_userid, mem_denied, mem_email_cert');
			if ( ! element('mem_id', $dbmember)) {
				...
				} else if (element('mem_denied', $dbmember) ==2) {
					$view['view']['error_message'] = '회원님의 아이디는 차단된 아이디입니다';
				}
			} else if ($this->cbconfig->item('use_register_email_auth') && ! element('mem_email_cert', $dbmember)) {
		...
		$config = array(
			array('field'=>'new_password', 'label'=>'패스워드', 'rules'=>'trim|required|min_length[' . $password_length . ']|callback__mem_password_check'),
			array('field'=>'new_password_re', 'label'=>'패스워드', 'rules'=>'trim|required|min_length[' . $password_length . ']'),
		);
		...
		*/
		if ($this->form_validation->run() == FALSE) {
			...
			if ($view['view']['error_message'] == '') {
				...
				$this->_update_login_log(element('mem_id', $result), 1, '패스워드 변경 후 로그인 성공');
