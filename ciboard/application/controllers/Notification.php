...
class Notification extends CB_Controller
{
	...
	*/
	public function index()
	{
		...
		*/
		$result = $this->Notification_model->get_notification_list($per_page, $offset, $this->member->item('mem_id'), $this->input->get('read'));
	...
	*/
	public function read($not_id='')
	{
		...
		if ( ! is_numeric($not_id) OR $not_id < 1) {
			show_404();
		}
		...
		if (element('mem_id', $notification) != $this->member->item('mem_id')) {
			show_404();
		}

		$this->Notification_model->mark_read($not_id, $this->member->item('mem_id'));
	...
	*  알림 읽기 입니다
	*/
	public function readajax($not_id='')
	{
		...
		*/
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				...
				$html .= '<div class="notifications-list"><a href="' . $read_url . '" ' . $jsevent . ' class="notification_read ' . element('not_type', $val) . '" data-not-id="' . element('not_id', $val) . '">' . html_escape(element('not_message', $val)) . ' <span class="pull-right">' . display_datetime(element('not_datetime', $val),'sns') .'</span></a></div> ';
			}
		}
		$html .= '<div class="notifications-list"><a class="external point" href="' . site_url('notification') . '">알림 페이지로 이동</a></div>';
		echo $html;
		exit();
