...
class Note extends CB_Controller
{
	...
	*/
	...
	function __construct()
	{
		...
		*/
		...
		if ($this->member->item('meta_note_received_datetime')) {
			$metadata = array(
	...
	*/
	public function lists($type ='recv')
	{
		...
		if ($this->cbconfig->item('use_note') == '') {
			...
			return FALSE;
		} else if ( ! $this->member->item('mem_use_note') &&  $this->member->is_admin() != 'super') {
		...
		*/
		...
		if ($this->cbconfig->get_device_view_type() == 'mobile') {
			$per_page = $this->cbconfig->item('note_mobile_list_page') ? $this->cbconfig->item('note_mobile_list_page') : 10;
		} else {
			$per_page = $this->cbconfig->item('note_list_page') ? $this->cbconfig->item('note_list_page') : 10;
		}
		...
		*/
		$where = array($my_column => $this->member->item('mem_id'), 'nte_type' => $nte_type);
		if ($type == 'send') {
			$result = $this->Note_model->get_send_list($per_page, $offset, $where , '' , $findex, $forder);
		} else {
			$result = $this->Note_model->get_recv_list($per_page, $offset, $where , '' , $findex, $forder);
		}
	...
	* 쪽지 상세보기 페이지입니다
	*/
	public function view($type = 'recv' , $note_id='')
	{
		...
		if ( ! $note_id OR ! is_numeric($note_id)) {
			show_404();
		}


		$where = array('nte_id' => $note_id, $columnname => $this->member->item('mem_id'), 'nte_type' => $nte_type);
		...
		if ($result[$mem_column]) {
			...
			$result['display_name'] =  display_username(element('mem_userid', $result), element('mem_nickname', $result), element('mem_icon', $result));
		} else {
			$result['display_name'] =  '알림';
		}
		if (element('nte_type', $result) == '1' && (empty($result['nte_read_datetime']) OR $result['nte_read_datetime'] <= '0000-00-00 00:00:00')) {
			$updatedata = array(
			...
			$this->Member_meta_model->save($this->member->item('mem_id'), $updatedata);

		}
		$result['content'] = display_html_content(element('nte_content', $result), element('nte_content_html_type', $result), $thumb_width = 500, $autolink = TRUE, $popup = TRUE);
		...
		*/
		...
		$searchconfig = array('{쪽지제목}');
		$replaceconfig = array(element('nte_title', $result));
	...
	*/
	public function write($userid = '')
	{
		...
		if ($this->cbconfig->get_device_view_type() != 'mobile' && $this->cbconfig->item('use_note_dhtml')) {
			$view['view']['use_dhtml'] = TRUE;
		}
		if ($this->cbconfig->get_device_view_type() == 'mobile' && $this->cbconfig->item('use_note_mobile_dhtml')) {
			$view['view']['use_dhtml'] = TRUE;
		}
		/**
		...
		*/
		$config = array(
			array('field'=>'userid', 'label'=>'회원아이디', 'rules'=>'trim|required|callback__check_userid'),
		...
		*/
		if ($this->form_validation->run() == FALSE) {
				...
				foreach ($recv_list as $key => $value) {
					...
					if ($value) {
						...
						if (element('mem_id', $mem)) {
							$send_result = $this->notelib->send_note($this->member->item('mem_id'), element('mem_id', $mem) , $this->input->post('title'), $this->input->post('content'), $content_type);
			...
			if ($view['view']['message'] == '') {
				...
				$this->session->set_flashdata('message', '쪽지가 발송되었습니다');
	...
	public function _check_userid($str)
	{
		...
		if ($recv_list && is_array($recv_list)) {
			foreach ($recv_list as $key => $value) {
				$value = trim($value);
				if ($value) {
					$mem = $this->Member_model->get_by_userid($value, 'mem_id, mem_denied, mem_use_note, mem_userid, mem_nickname');
					if (element('mem_id', $mem) && element('mem_denied', $mem) == 0 && element('mem_use_note', $mem) == 1) {
		...
		if ($error_msg && $this->member->is_admin() != 'super') {
			$this->form_validation->set_message('_check_userid', $error_msg . ' 는 쪽지를 받을 수 없는 회원아이디입니다. <br />쪽지를 발송하지 않았습니다');
			return FALSE;
		}
		if ($this->member->is_admin() != 'super') {
			if (count(element('mem_id', $mem_list))) {
				$point = (int)$this->cbconfig->item('point_note') * count(element('mem_id', $mem_list));
				if ($point) {
					if ($this->member->item('mem_point') - $point < 0) {
						$this->form_validation->set_message('_check_userid', '보유하신 포인트(' . number_format($this->member->item('mem_point')) . '점)가 모자라서 쪽지를 보낼 수 없습니다');
						return FALSE;
					}
				}
			}
	...
	* 쪽지를 삭제하는 함수입니다
	*/
	public function delete($note_type = 'recv', $note_id = '')
	{
		...
		if ( ! is_numeric($note_id) OR $note_id < 1) {
			show_404();
		}
		...
		if (element('nte_type', $note) == '1') {
			...
			if (element('recv_mem_id', $note) != $this->member->item('mem_id')) {
				show_404();
			}
			$note_list = 'recv';
		} else if (element('nte_type', $note) == '2') {
			if ($note_type != element('nte_type', $note)) {
				show_404();
			}
			if (element('send_mem_id', $note) != $this->member->item('mem_id')) {
				show_404();
			}
			$note_list = 'send';
		}
