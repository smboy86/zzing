...
class Comment_model extends CB_Model {
	...
	*/
	...
	public function get_admin_list($limit = '', $offset = '', $where = '', $like = '', $findex = '', $forder = '', $sfield = '', $skeyword = '', $sop='OR')
	{
		...
		$select = 'comment.*, post.mem_id as post_mem_id, post.post_userid, post.post_username, post.post_nickname, post.brd_id, post.post_datetime, post.post_hit, post.post_secret, post.post_title';
	...
	*  List 페이지 커스테마이징 함수
	*/
	function get_comment_list($limit = '', $offset = '', $where = '', $like = '', $orderby = '', $sfield = '', $skeyword = '', $sop='OR')
	{

		if ( ! in_array(strtolower($orderby), $this->allow_order)) $orderby = "cmt_num, cmt_reply";
		$sop = (strtoupper($sop) == 'AND') ? 'AND' : 'OR';
		if ( ! $sfield) $sfield = 'cmt_content';
				...
				if ($skeyword && $ssf && in_array($ssf, $this->allow_search_field)) {
					if (in_array($ssf , $this->search_field_equal)) {
						...
						foreach ($swordarray as $str) {
							if ($ssf == '') continue;
							if ($sop == 'AND') { $search_like[] = array($ssf => $str); } else { $search_or_like[] = array($ssf => $str); }
						}
					}
				}
			}
	...
	public function get_latest($where = '', $like = '', $limit='', $findex = 'cmt_id', $forder = 'DESC')
	{
		...
		$this->db->select('comment.*, post.mem_id as post_mem_id, post.post_userid, post.post_username, post.post_nickname, post.brd_id, post.post_datetime, post.post_hit, post.post_secret, post.post_title');
	...
	public function next_comment_num()
	{
		...
		$row['cmt_num'] = ( isset($row['cmt_num'])) ? $row['cmt_num'] : 0;
		$cmt_num = $row['cmt_num'] - 1;
		return $cmt_num;

	}

	public function count_reply_comment($cmt_id='', $post_id='', $cmt_num='', $cmt_reply='')
	{
		$where = array(
			'cmt_id <>' => $cmt_id,
			'post_id' => $post_id,
