...
class Post_model extends CB_Model {
	...
	*/
	public function get_post_list($limit = '', $offset = '', $where = '', $category_id = '', $orderby = '', $sfield = '', $skeyword = '', $sop='OR')
	{
		...
		if ( ! in_array(strtolower($orderby), $this->allow_order)) $orderby = "post_num, post_reply";

		$sop = (strtoupper($sop) == 'AND') ? 'AND' : 'OR';
		if ( ! $sfield) $sfield = array('post_title', 'post_content');
	...
	function get_notice_list($brd_id ='', $except_all_notice='', $sfield = '', $skeyword = '', $sop='OR')
	{
	...
	*  List 페이지 커스테마이징 함수
	*/
	public function get_prev_next_post($post_id = '', $post_num ='', $type = '', $where = '', $sfield = '', $skeyword = '', $sop='OR')
	{

		if ( ! $post_id OR ! is_numeric($post_id)) return FALSE;
		...
		$orderby = ($type == 'next') ? 'post_num, post_reply' : 'post_num desc, post_reply desc';
	...
	*  List 페이지 커스테마이징 함수
	*/
	function get_search_list($limit = '', $offset = '', $where = '', $like = '', $board_id = '', $orderby = '', $sfield = '', $skeyword = '', $sop='OR')
	{
		...
		$this->db->select('post.*, board.brd_key, board.brd_name, board.brd_mobile_name, board.brd_order, board.brd_search, member.mem_id, member.mem_userid, member.mem_nickname, member.mem_icon, member.mem_photo, member.mem_point ');
		$this->db->from('post');
		...
		if ($board_id)	{ $this->db->where( array('b.brd_id' => $board_id)); }
	...
	function get_rss_list($where='', $where_in='' , $limit ='', $offset='')
	{

		if ($where) $this->db->where($where);
		if ($where_in) $this->db->where_in('brd_id', $where_in);
		$this->db->order_by('post_num, post_reply');
		if ($limit) $this->db->limit($limit, $offset);
