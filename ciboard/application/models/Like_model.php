...
class Like_model extends CB_Model {
	...
	*/
	...
	{
		$select = 'like.*, post.mem_id as post_mem_id, post.post_userid, post.post_username, post.post_nickname, post.post_id, post.brd_id, post.post_datetime, post.post_hit, post.post_secret, post.post_title, post.post_like, post.post_dislike, post.post_image, post.post_del';
	...
	{
		$select = 'like.*, comment.cmt_id, comment.mem_id as cmt_mem_id, comment.cmt_userid, comment.cmt_username, comment.cmt_nickname, comment.post_id, comment.cmt_datetime, comment.cmt_secret, comment.cmt_content, comment.cmt_content, comment.cmt_like, comment.cmt_dislike, comment.cmt_del';
	...
	function get_like_count($type = 'd', $start_date='' , $end_date='', $brd_id='')
	{
		...
		$this->db->order_by('lik_datetime', 'desc');
	...
	function get_dislike_count($type = 'd', $start_date='' , $end_date='', $brd_id='')
	{
		if ( ! $start_date) return FALSE;
		if ( ! $end_date) return FALSE;
