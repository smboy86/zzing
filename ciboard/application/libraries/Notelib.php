...
class Notelib extends CI_Controller
{
	...
	public function send_note($sender='', $receiver='' , $title='', $content='', $content_type ='')
	{
		...
		$point_note = $this->CI->cbconfig->item('use_point') ? $this->CI->cbconfig->item('point_note') : 0;

		$send_member = '';
		if ($sender) {
			$send_member = $this->CI->Member_model->get_by_memid($sender, 'mem_id, mem_point, mem_use_note, mem_is_admin, mem_nickname');
		...
		$recv_member = '';
		if ($receiver) {
			$recv_member = $this->CI->Member_model->get_by_memid($receiver, 'mem_id, mem_point, mem_use_note, mem_nickname, mem_denied');
		...
		if ($sender) {
			...
			if ($this->CI->cbconfig->item('use_point') && $point_note && $point_note > 0) {
				...
				$this->CI->point->insert_point($sender, $point, element('mem_nickname' , $recv_member) . '님께 쪽지 발송', 'note', $note_id, $note_id . ' 쪽지 발송');
			}
		}
		...
		if ($this->CI->cbconfig->item('use_notification') && $this->CI->cbconfig->item('notification_note')) {
			...
			$return = $this->CI->notificationlib->set_noti($receiver, $sender , 'note', $note_id, $not_message, $not_url);
		}
