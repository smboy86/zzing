...
class Managelayout extends CI_Controller
{
	...
	function admin($config=array(), $device_view_type = '')
	{
		$data = array();
		 ...
		 $data['admin_page_menu'] = $CI->config->item('admin_page_menu');
	...
	function front($config=array(), $device_view_type = '')
	{
		...
		if ( ! $layoutdirname) $layoutdirname = $device_view_type == 'mobile' ? $CI->cbconfig->item('mobile_layout_default') : $CI->cbconfig->item('layout_default');
		if ( ! $layoutdirname) $layoutdirname = 'basic';
		...
		if ( ! $skindir) $skindir = $device_view_type == 'mobile' ? $CI->cbconfig->item('mobile_skin_default') : $CI->cbconfig->item('skin_default');
		...
		if($user_sidebar == '1') { $data['use_sidebar'] = '1'; }
		else if($user_sidebar == '2') { $data['use_sidebar'] = ''; }
		else {
		...
		$CI->Currentvisitor_model->add_visitor($CI->input->ip_address(), $CI->member->item('mem_id'),
		$CI->member->item('mem_nickname'), cdate('Y-m-d H:i:s'), $currentpage, current_full_url(), $CI->agent->referrer(), $CI->agent->agent_string());
		if ($CI->cbconfig->item('open_currentvisitor') OR $CI->member->is_admin() == 'super') {

			$minute = $CI->cbconfig->item('currentvisitor_minute') ;
			if ( ! $minute OR ! is_numeric($minute) OR $minute < 0) $minute = 10;
		...
		if ($CI->cbconfig->item('use_notification')) {
			if ($CI->member->is_member()) {
				...
				$data['notification_num'] = $CI->Notification_model->unread_notification_num($CI->member->item('mem_id'));
			}
		}
