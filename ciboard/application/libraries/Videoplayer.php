...
class Videoplayer extends CI_Controller
{
	...
	function get_video($vid='')
	{
		...
		if (element('type', $video) == "file") { //JWPLAYER
			...
			} else if (element('type', $video) == "vimeo") { //비메오
				$autoplay = (element('auto', $video)) ? '&amp;autoplay=1' : '';
			...
			} else if (element('type', $video) == "ted") { //테드
			...
			} else if (element('type', $video) == "daum") { //다음TV
				$autoplay = (element('auto', $video)) ? '&autoPlay=1' : '';
			...
			} else if (element('type', $video) == "dailymotion") { //Dailymotion
			...
			} else if (element('type', $video) == "pandora") { //판도라TV
				if (element('auto', $video)) $auto = "&amp;autoPlay=true";
			...
			} else if (element('type', $video) == "nate") { //네이트TV
				$autoplay = (element('auto', $video)) ? '&autoPlay=1' : '';
			...
			} else if (element('type', $video) == "tagstory") { //Tagstory
			...
			} else if (element('type', $video) == "slidershare") { // SliderShare
			...
			} else if (element('type', $video) == "facebook") { // Facebook - 라니안님 코드 반영
			...
			} else if (element('type', $video) == "naver") { // Naver - 라니안님 코드 반영
			...
			} else if (element('type', $video) == "tvcast") { // Naver Tvcast - 라니안님 코드 반영
				$autoplay = (element('auto', $video)) ? '&isp=1' : '';
	...
	function get_video_info($video_url)
	{
		...
		if (element('host', $info) == "youtu.be") { //유튜브
		...
		} else if (element('host', $info) == "www.youtube.com") { //유튜브
			$video['type'] = 'youtube';
		...
		} else if (element('host', $info) == "vimeo.com") { //비메오
			$video['type'] = 'vimeo';
			$vquery = explode("/",element('video_url', $video));
			$num = count($vquery) - 1;
			list($video['vid']) = explode("#", element($num, $vquery));
		} else if (element('host', $info) == "www.ted.com") { //테드
			$video['type'] = 'ted';
			$vquery = explode("/",element('video_url', $video));
			$num = count($vquery) - 1;
			list($video['vid']) = explode(".", element($num, $vquery));
			$video['rid'] = trim(element('path', $info));
		} else if (element('host', $info) == "tvpot.daum.net") { //다음tv
			$video['type'] = 'daum';
		...
		} else if (element('host', $info) == "channel.pandora.tv") { //판도라tv
			$video['type'] = 'pandora';
			...
			if ( ! is_numeric(element('vid', $video))) $video = NULL;
		} else if (element('host', $info) == "serviceapi.nmv.naver.com") { // 네이버
			$video['type'] = 'naver';
			$video['vid'] = element('vid', $query);
			$video['outKey'] = element('outKey', $query);
		} else if (element('host', $info) == "serviceapi.rmcnmv.naver.com") { // 네이버
			$video['type'] = 'tvcast';
			$video['vid'] = element('vid', $query);
			$video['outKey'] = element('outKey', $query);
		} else if (element('host', $info) == "tvcast.naver.com") {
			$video['type'] = 'tvcast';
		...
		} else if (element('host', $info) == "www.slideshare.net") { // slidershare
	...
	function get_video_id($url, $vid, $type)
	{
		...
		if ( ! $url OR ! $vid OR ! $type OR ($type == 'file')) return;
