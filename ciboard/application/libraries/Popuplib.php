		...
		*/
		...
		if (element('list', element('view', $view))) {
			foreach (element('list', element('view', $view)) as $key => $value) {
				if (get_cookie('popup_layer_' . element('pop_id', $value))) continue;
				if ($this->CI->cbconfig->get_device_view_type() == 'mobile' && element('pop_device', $value) == 'pc') continue;
				if ($this->CI->cbconfig->get_device_view_type() != 'mobile' && element('pop_device', $value) == 'mobile') continue;
				if ( ! element('pop_page', $value) && $this->CI->uri->segment(1)) continue;
